...
function assertParsableJsFinal(code: string): void {
  try {
    new Script(code, { filename: 'production-cleanup:final' });
  } catch (error) {
...
function stripEmptyEsmMinInitWrappers(code: string): string {
  ...
  if (removedNames.size === 0) return result;
...
function removeLogCalls(code: string, methods: string[]): string {
  ...
  while ((match = regex.exec(code)) !== null) {
    ...
    if (depth === 0) {
      ...
      result += shouldUseVoidExpression ? 'void 0' : ';';
      let endIndex = i;
      const hadSemicolon = code[endIndex] === ';';
      if (hadSemicolon) {
        if (shouldUseVoidExpression) result += ';';
        endIndex++;
      }
      while (
        code[endIndex] === '\n' ||
...
function splitLeadingUserscriptHeader(source: string): { header: string; body: string } {
  if (!source.startsWith('// ==UserScript==')) return { header: '', body: source };
