...
(function() {
	...
	*/
	...
	var MV3DownloadAdapter = class {
		...
		async download(url, filename, headers) {
			const error = unwrapResponse(await browserApi.runtime.sendMessage({
	...
	*/
	var getDownloadAdapter = createAdapter(() => new MV3DownloadAdapter(), () => {
		...
		return {
			download: (url, filename) => api.download(url, filename),
			downloadBlob: (blob, filename) => api.downloadBlob(blob, filename),
	...
	*/
	...
	async function apiRequest(url, location) {
	...
	*/
	async function getTweetMedias(tweetId, location) {
		const json = await apiRequest(createTweetEndpointUrl(tweetId, location), location);
	...
	*/
	...
	var Tooltip_module_default = {
		trigger: "_trigger_1ioo5_11",
		content: "_content_1ioo5_12",
		placementBottom: "_placementBottom_1ioo5_53",
		placementTop: "_placementTop_1ioo5_57",
		visible: "_visible_1ioo5_62",
		arrow: "_arrow_1ioo5_13",
		placementBottomArrow: "_placementBottomArrow_1ioo5_76",
		placementTopArrow: "_placementTopArrow_1ioo5_84",
		arrowInner: "_arrowInner_1ioo5_93",
		placementBottomArrowInner: "_placementBottomArrowInner_1ioo5_101",
		placementTopArrowInner: "_placementTopArrowInner_1ioo5_109"
	};
	//#endregion
	...
	*/
	...
	var Toolbar_module_default = {
		toolbarButton: "_toolbarButton_1224b_17",
		galleryToolbar: "_galleryToolbar_1224b_16",
		settingsExpanded: "_settingsExpanded_1224b_155",
		tweetPanelExpanded: "_tweetPanelExpanded_1224b_156",
		stateIdle: "_stateIdle_1224b_164",
		stateLoading: "_stateLoading_1224b_165",
		stateDownloading: "_stateDownloading_1224b_166",
		stateError: "_stateError_1224b_167",
		toolbarContent: "_toolbarContent_1224b_174",
		toolbarControls: "_toolbarControls_1224b_18",
		counterBlock: "_counterBlock_1224b_18",
		separator: "_separator_1224b_204",
		downloadButton: "_downloadButton_1224b_21",
		mediaCounterWrapper: "_mediaCounterWrapper_1224b_241",
		mediaCounter: "_mediaCounter_1224b_19",
		currentIndex: "_currentIndex_1224b_266",
		totalCount: "_totalCount_1224b_271",
		progressBar: "_progressBar_1224b_19",
		progressFill: "_progressFill_1224b_290",
		fitButton: "_fitButton_1224b_20",
		closeButton: "_closeButton_1224b_342",
		settingsPanel: "_settingsPanel_1224b_22",
		tweetPanel: "_tweetPanel_1224b_23",
		panelExpanded: "_panelExpanded_1224b_356",
		tweetUrlLink: "_tweetUrlLink_1224b_364",
		tweetContent: "_tweetContent_1224b_365",
		tweetPanelBody: "_tweetPanelBody_1224b_433",
		tweetTextHeader: "_tweetTextHeader_1224b_440",
		tweetTextLabel: "_tweetTextLabel_1224b_448",
		tweetUrlSection: "_tweetUrlSection_1224b_535",
		tweetUrlIcon: "_tweetUrlIcon_1224b_579",
		tweetUrlLabel: "_tweetUrlLabel_1224b_586",
		tweetUrlValue: "_tweetUrlValue_1224b_592",
		tweetUrlDivider: "_tweetUrlDivider_1224b_600"
	};
	//#endregion
	...
	*/
	...
	async function downloadWithAdapter(url, filename, options, abortSignal) {
		...
		if (abortSignal) return raceWithAbort(adapter.download(url, filename).then(() => {
		...
		try {
			...
			if (abortSignal) {
				...
				try {
					...
					const result = await raceWithAbort(adapter.downloadBlob(blob, filename).then(() => ({
						success: true,
						filename
					}), (error) => createErrorDownloadResult(error)), abortSignal, createAbortResult);
					if (!result.success) return result;
	...
	*/
	...
	var DownloadOrchestrator = class {
		...
		async downloadBulk(mediaItems, options = {}) {
			...
			try {
				...
				try {
					...
					if (!saveResult.success) return createErrorResponse(saveResult.error || "Failed to save ZIP file", ErrorCode.ALL_FAILED, items.length, {
		...
		* @internal
		*/
		async saveWithDownloadAdapter(blob, filename) {
			const adapter = getDownloadAdapter();
			try {
				await adapter.downloadBlob(blob, filename);
				return { success: true };
	...
	*/
	...
	function GalleryContainer(props) {
		...
		const ariaHiddenRestore = /* @__PURE__ */ new Map();
		createEffect(() => {
		...
		onCleanup(() => {
			...
			for (const el of hiddenBackgroundElements) {
				...
				if (previous === null || previous === void 0) el.removeAttribute("aria-hidden");
	...
	*/
	...
	function GalleryRoot(props) {
		...
		const handleRenderError = () => {
			...
			for (const el of Array.from(document.body.children)) if (el instanceof HTMLElement && !el.hasAttribute("data-xeg-gallery-container") && !el.hasAttribute("data-renderer")) el.removeAttribute("aria-hidden");
		};
	...
	*/
	...
	if (typeof navigation !== "undefined") navigation.addEventListener("navigate", () => {
		if (isAllowedStartPage() && !lifecycleState.started) startApplication().catch((error) => {});
		else if (!isAllowedStartPage() && lifecycleState.started) cleanup().catch((error) => {});
	});
