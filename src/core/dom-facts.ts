 ...
 *
 * ## Purpose
 * Defines type structures for DOM facts - structured snapshots of page state extracted from
 * the DOM at runtime. DOM facts provide a type-safe way to communicate page information from
 * content scripts to the application logic layer without tight coupling to DOM implementation details.
 *
 * ## Key Responsibilities
 * - **Type Safety**: Provide compile-time type checking for DOM fact structures
 * - **State Snapshot**: Define immutable snapshots of page state at specific points in time
 * - **Decoupling**: Abstract DOM implementation details from business logic
 * - **Extensibility**: Support multiple fact kinds for different page contexts
 *
 * ## Architecture Context
 * DOM facts are part of the effect system's fact extraction pipeline:
 * 1. Business logic dispatches TAKE_DOM_FACTS command
 * 2. Runtime adapter extracts DOM information
 * 3. Runtime creates DomFacts object from extracted data
 * 4. DomFacts object returned to business logic layer
 *
 * ## Fact Kinds
 * Currently supported fact kinds:
 * - **XComGallery**: Facts about X.com gallery page state (overlay presence, media elements, etc.)
 *
 * Future extensibility:
 * - XComTimeline: Timeline page facts
 * - XComProfile: Profile page facts
 * - XComTweet: Individual tweet facts
 *
 * ## Design Principles
 * ### Immutability
 * All fact properties are `readonly` to ensure snapshots remain unchanged after creation.
 * This prevents accidental mutations and enables caching/memoization.
 *
 * ### Kind Discriminator
 * The `kind` field acts as a discriminator for exhaustive pattern matching, allowing
 * type-safe handling of different fact variants as the system grows.
 *
 * ### Minimal Data
 * Facts contain only essential information needed by business logic, avoiding
 * unnecessary DOM details that would couple implementation to DOM structure.
 *
 * ## Usage Pattern
 * ```typescript
 ...
 *
 * // Response handling
 * function handleFacts(facts: DomFacts): void {
 *   switch (facts.kind) {
 *     case 'XComGallery':
 *       console.log('Gallery overlay:', facts.hasXegOverlay);
 *       console.log('Media elements:', facts.mediaElementsCount);
 *       break;
 *     default:
 *       const _exhaustive: never = facts.kind;
 *       throw new Error(`Unhandled fact kind: ${_exhaustive}`);
 *   }
 * }
 * ```
 *
 * @module core/dom-facts
 * @see {@link DomFacts} for the main facts interface
 * @see {@link DomFactsKind} for supported fact kinds
 * @see {@link module:core/cmd} for related TAKE_DOM_FACTS command
 */
 ...
 *
 * @remarks
 * Acts as a type-safe discriminator for pattern matching on different DOM fact kinds.
 * Each kind represents a distinct page context with its own set of extracted information.
 *
 * **Current Kinds**:
 * - `XComGallery`: Facts about X.com gallery page state (enhanced gallery overlay, media viewer, media elements)
 *
 * **Future Extensions**:
 * When adding new fact kinds, update this union type and extend the DomFacts interface
 * to support the new variant with appropriate discriminated union patterns.
 *
 * @example
 * ```typescript
 * // Type-safe kind checking
 * function processFactsKind(kind: DomFactsKind): void {
 *   switch (kind) {
 *     case 'XComGallery':
 *       console.log('Processing gallery facts');
 *       break;
 *     default:
 *       // TypeScript ensures exhaustiveness
 *       const _exhaustive: never = kind;
 *       throw new Error(`Unknown fact kind: ${_exhaustive}`);
 *   }
 * }
 *
 * // Runtime validation
 * function isValidFactsKind(value: unknown): value is DomFactsKind {
 *   return typeof value === 'string' && value === 'XComGallery';
 * }
 * ```
 */
