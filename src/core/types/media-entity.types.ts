 ...
 * 파일명 값 객체
 */
export class MediaFilename implements ValueObject<string> {
  readonly value: string;
  ...
  toSafeFilename(): MediaFilename {
    ...
    return new MediaFilename(safeName);
  }
}
...
export class MediaEntity implements DomainEntity {
  ...
  readonly filename: MediaFilename;
  ...
  constructor(config: {
    ...
    // 파일명 처리
    if (config.filename) {
      this.filename = new MediaFilename(config.filename);
    } else {
   ...
   * 파일명 자동 생성
   */
  private generateFilename(): MediaFilename {
    ...
    try {
      ...
      if (lastSegment?.includes('.')) {
        return new MediaFilename(lastSegment);
      }
    } catch {
    ...
    return new MediaFilename(defaultName);
  }
