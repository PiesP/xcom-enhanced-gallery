...
export class GalleryStateManager {
  ...
  closeGallery(): void {
    ...
    // 캐시 정리: VideoService의 상태도 함께 정리
    try {
      // const videoService = VideoService.getInstance();
      // videoService.clearAllCache(); // TODO: 캐시 기능 추가 필요
      logger.debug('Gallery close: video service cleared');
    } catch (error) {
      logger.warn('Failed to clear video service during gallery close:', error);
    }
