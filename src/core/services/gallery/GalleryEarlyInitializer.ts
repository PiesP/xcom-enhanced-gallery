...
export class GalleryEarlyInitializer {
  ...
  private async performEarlyInitialization(): Promise<void> {
    ...
    try {
      // 1. 스크롤 관리자 초기화
      await this.initializeScrollManager();
   ...
   * 스크롤 관리자 초기화
   */
  private async initializeScrollManager(): Promise<void> {
    try {
      if (!this.serviceManager) {
        throw new Error('Service manager not available');
      }

      const scrollManager = await this.serviceManager.tryGet(SERVICE_KEYS.SCROLL_MANAGER);
      if (scrollManager) {
        await this.waitForServiceReady(scrollManager, 'isReady');
        this.logDebug('Scroll manager initialized');
      }

      this.state.scrollManagerReady = true;
    } catch (error) {
      this.logError('Failed to initialize scroll manager', error);
      // 스크롤 관리자 실패는 치명적이지 않으므로 계속 진행
      this.state.scrollManagerReady = true;
    }
  }
  ...
  private async initializeScrollLockService(): Promise<void> {
    try {
      ...
      // ScrollManager를 통한 스크롤 잠금 서비스 준비
      const scrollLockService = await this.serviceManager.tryGet(SERVICE_KEYS.SCROLL_MANAGER);
      if (!scrollLockService) {
        this.logDebug('Scroll lock service not available, using fallback');
        this.state.scrollLockReady = true;
        return;
      }

      // ScrollManager는 즉시 사용 가능하므로 준비 상태 확인
      await this.waitForServiceReady(scrollLockService, 'isReady', 1000);
      ...
      // 스크롤 잠금 서비스 실패도 치명적이지 않으므로 계속 진행
      this.state.scrollLockReady = true;
    }
  }
  ...
  private async initializeGalleryService(): Promise<void> {
    try {
      ...
      if (galleryService) {
        await this.waitForServiceReady(galleryService, 'isReady', 1000);
  ...
  private async performInvisibleGalleryWarmup(): Promise<void> {
    try {
      ...
      // 간단한 더미 Warmup - ScrollManager 준비만 확인
      const scrollManager = await this.serviceManager.tryGet(SERVICE_KEYS.SCROLL_MANAGER);
      if (scrollManager && typeof scrollManager === 'object') {
        // ScrollManager의 기본 메서드 호출로 초기화 확인
        if ('getInstance' in scrollManager && typeof scrollManager.getInstance === 'function') {
          scrollManager.getInstance();
          this.logDebug('ScrollManager warmup 완료');
        }
      }
