/**
 * @fileoverview 간소화된 미디어 추출기
 * @description API 우선 + DOM 백업 2단계 전략만 사용하는 단순화된 추출기
...
export class MediaExtractionService implements MediaExtractor {
  ...
  async extractFromClickedElement(
    ...
    logger.info(`[SimplifiedExtractor] ${extractionId}: 추출 시작`);
    ...
    try {
      ...
      if (!tweetInfo?.tweetId) {
        logger.debug(
          `[SimplifiedExtractor] ${extractionId}: 트윗 정보 없음 - DOM 직접 추출로 진행`
        );
      ...
      logger.debug(`[SimplifiedExtractor] ${extractionId}: 트윗 정보 확보 - ${tweetInfo.tweetId}`);
      ...
      if (apiResult.success && apiResult.mediaItems.length > 0) {
        logger.info(
          `[SimplifiedExtractor] ${extractionId}: ✅ API 추출 성공 - ${apiResult.mediaItems.length}개 미디어`
        );
        // core 인터페이스 형식으로 변환
        return {
      ...
      // 3단계: DOM 백업 추출
      logger.warn(`[SimplifiedExtractor] ${extractionId}: API 추출 실패 - DOM 백업 전략 실행`);
      ...
      logger.error(`[SimplifiedExtractor] ${extractionId}: 추출 실패:`, error);
  ...
  async extractAllFromContainer(
    ...
    const extractionId = this.generateExtractionId();
    logger.debug(`[SimplifiedExtractor] ${extractionId}: 컨테이너 추출 시작`);
    ...
    try {
      ...
      if (!firstMedia) {
        logger.warn(`[SimplifiedExtractor] ${extractionId}: 컨테이너에서 미디어를 찾을 수 없음`);
        return this.createErrorResult('컨테이너에서 미디어를 찾을 수 없음');
      }

      return this.extractFromClickedElement(firstMedia, options);
    } catch (error) {
      logger.error(`[SimplifiedExtractor] ${extractionId}: 컨테이너 추출 실패:`, error);
      return this.createErrorResult(error);
    }
  }
  ...
  private generateExtractionId(): string {
    return `simp_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  }
