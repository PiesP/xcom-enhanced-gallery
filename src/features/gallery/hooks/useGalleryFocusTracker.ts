...
export function useGalleryFocusTracker({
  ...
  /**
   * Recomputes the focused item index based on IntersectionObserver entries.
   *
   * Algorithm:
   * 1. Sort candidates by distance from container center
   * 2. Sort by intersection ratio (secondary)
   * 3. Sort by entry time (tertiary)
   *
   * Falls back to currentIndex if no items are visible.
   */
  ...
  /**
   * Registers or unregisters a gallery item for focus tracking.
   *
   * @param index Gallery item index
   * @param element DOM element to register (null to unregister)
   *
   * On each call, recomputeFocus() is invoked to re-evaluate current focus.
   */
  ...
  onCleanup(() => {
    ...
    setFocusTracking(createFocusTracking()); // Reset tracking state
  });
