...
interface useToolbarReturn {
  ...
  isVisible: boolean;
  /** 호버 존 ref */
  hoverZoneRef: { current: HTMLDivElement | null };
}
...
export function useToolbar({
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      const hoverZone = hoverZoneRef.current;
      if (hoverZone) {
        const toolbarWrapper = hoverZone.parentElement?.querySelector('[class*="toolbarWrapper"]');
        if (toolbarWrapper) {
          const hoverZIndex = parseInt(getComputedStyle(hoverZone).zIndex);
          const toolbarZIndex = parseInt(getComputedStyle(toolbarWrapper).zIndex);

          if (hoverZIndex >= toolbarZIndex) {
            logger.warn('Z-Index conflict detected: hover zone should be below toolbar', {
              hoverZIndex,
              toolbarZIndex,
            });
          } else {
            logger.debug('Z-Index hierarchy verified', {
              hoverZIndex,
              toolbarZIndex,
              isValid: toolbarZIndex > hoverZIndex,
            });
          }
        }
      }
