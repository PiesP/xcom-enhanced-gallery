 ...
 *
 * Custom Solid.js hook that manages keyboard event handling for the gallery overlay.
 * Provides responsive keyboard navigation (Escape to close) while respecting editable
 * form fields (INPUT, TEXTAREA, contenteditable elements).
 *
 * **Purpose & Architecture**
 * - Registers a global keydown listener on document (capture phase)
 * - Filters events based on target element (skip editable fields)
 * - Executes onClose callback when Escape key is pressed
 * - Automatically cleans up event listener on component unmount
 * - Uses EventManager singleton for centralized event handling
 *
 * **Hook Lifecycle**
 * Setup phase: EventManager registers document keydown listener
 * User action: handleKeyDown filters editable targets, calls onClose
 * Cleanup phase: EventManager removes listener on component unmount
 *
 * **Design Pattern - Event Manager Delegation**
 * Instead of directly calling element.addEventListener(), we delegate to
 * EventManager singleton. This provides:
 * - Centralized listener tracking
 * - Automatic cleanup on context unload
 * - Built-in listener filtering and removal
 * - Context-aware routing (gallery-keyboard-navigation)
 *
 * **Key Design Principles**
 * 1. **Editable Field Awareness** - Don't intercept keyboard in text inputs
 * 2. **Non-Blocking Architecture** - Early return for irrelevant events
 * 3. **Capture Phase** - Listen in capture phase to intercept before bubbling
 * 4. **Explicit Cleanup** - onCleanup ensures no listener leaks
 * 5. **SSR Safety** - Check for document existence before registering
 *
 * **Accessibility Considerations**
 * - Escape key is standard for modal/overlay close (ARIA APG pattern)
 * - Respects user input in form fields (INPUT, TEXTAREA)
 * - Respects contenteditable regions for rich text editors
 * - Capture phase ensures keyboard works even if gallery has focus traps
 *
 * **Performance Notes**
 * - Event handler dispatch: O(1) - keydown events are low-frequency
 * - isEditableTarget check: O(1) - tagName check plus contentEditable check
 * - EventManager removal: O(1) - uses internal Set for listener tracking
 * - No memory leaks - onCleanup guarantees listener removal
 *
 * **Browser Compatibility**
 * - Escape key: all browsers (standard KeyboardEvent)
 * - EventTarget: ES5 standard
 * - contenteditable: HTML5 standard (IE9+)
 * - Capture phase: DOM Level 2 standard
 *
 * @remarks
 * This hook must be called during component render, not conditionally.
 * Solid.js effects have reactive dependencies, so calling this at
 * the top level ensures proper cleanup. The hook performs all work
 * in the createEffect block to ensure proper Solid.js integration.
 *
 * @example
 * Using the hook in a gallery component:
 * \`\`\`typescript
 ...
 *
 *   return (
 *     <Show when={galleryVisible()}>
 *       <div class={styles.overlay}>Gallery content</div>
 *     </Show>
 *   );
 * }
 * \`\`\`
 *
 * @example
 * Escape key behavior with input field:
 * - Scenario 1: Gallery visible, no input focused
 *   User presses Escape (onClose called, gallery closes)
 * - Scenario 2: Gallery visible, cursor in INPUT element
 *   User presses Escape (isEditableTarget returns true, early return, browser handles normally)
 * - Scenario 3: Gallery visible, cursor in contenteditable div
 *   User presses Escape (isEditableTarget detects isContentEditable, early return, browser handles normally)
 *
 * @see {@link UseGalleryKeyboardOptions} - Configuration interface
 * @see {@link EventManager} from '@shared/services/event-manager' - Event delegation
 * @see {@link createEffect} from 'solid-js' - Solid.js effect hook
 */
