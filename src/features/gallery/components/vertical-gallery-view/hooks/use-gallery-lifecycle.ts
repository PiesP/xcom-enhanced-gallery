/**
 * @fileoverview Gallery Lifecycle Management Hook - Animation, Cleanup & Viewport Tracking
 *
 * Custom Solid.js hook that orchestrates the complete gallery lifecycle from opening
 * to closing. Manages three independent but coordinated responsibilities:
 * 1. Container scroll availability setup (immediate)
 * 2. Enter/exit animations with video cleanup (reactive to visibility)
 * 3. Viewport CSS variables synchronization (continuous observation)
 *
 * **Purpose & Architecture**
 * This hook encapsulates all gallery state transitions and effects into a single
 * composable unit. Instead of scattering effects across multiple components,
 * all lifecycle management is centralized here for clarity and reusability.
 *
 * **Three Independent Effects**
 * ```
 * Effect 1: Container Setup
 *   - Dependency: containerEl
 *   - Runs once when container reference changes
 *   - Sets up scroll availability for overflow container
 *
 * Effect 2: Animation & Cleanup
 *   - Dependencies: containerEl, isVisible
 *   - Runs when visibility toggles
 *   - Plays enter animation (on open)
 *   - Plays exit animation + cleans videos (on close)
 *
 * Effect 3: Viewport Tracking
 *   - Dependencies: containerEl, toolbarWrapperEl
 *   - Continuously observes toolbar height changes
 *   - Updates CSS variables for dynamic layout
 *   - Cleans up observer on unmount
 * ```
 *
 * **Design Pattern - Composition Over Methods**
 * Instead of a Gallery class with lifecycle methods:
 * \`\`\`typescript
 * class Gallery {
 *   mount() { ... }
 *   unmount() { ... }
 ...
 * \`\`\`
 *
 * We use Solid.js hooks for reactive composition:
 * \`\`\`typescript
 * useGalleryLifecycle({ containerEl, isVisible, ... })
 * \`\`\`
 *
 * Benefits: Declarative, testable, fine-grained reactivity, automatic cleanup.
 *
 * **Key Design Principles**
 * 1. **Reactive Dependencies** - Effects automatically track containerEl, isVisible
 * 2. **Error Resilience** - Video cleanup wrapped in try-catch (never crash)
 * 3. **Conditional Logging** - Development logging only via __DEV__ constant
 * 4. **Automatic Cleanup** - observeViewportCssVars cleanup guaranteed via onCleanup
 * 5. **Deferred Effects** - { defer: true } ensures CSS is applied, not queued
 *
 * **Performance Characteristics**
 * - Container setup: O(1) DOM manipulation (once per container change)
 * - Animation dispatch: O(1) class additions/CSS transitions
 * - Video cleanup: O(n) where n = number of videos (pause + reset)
 * - Viewport tracking: O(1) observer callback (runs on toolbar resize)
 * - Memory: No closures over component state (safe cleanup)
 *
 * **Browser Compatibility**
 * - Solid.js createEffect: ES5+ (all modern browsers)
 * - ResizeObserver: Chrome 64+, Safari 13.1+, Firefox 69+
 * - Video pause/currentTime: ES5 standard (all browsers)
 * - CSS transitions: CSS3 standard (all modern browsers)
 *
 * @remarks
 * This hook MUST be called at component top level (Solid.js rules).
 * All effects are automatically cleaned up when component unmounts.
 * Effects are idempotent: safe to call multiple times without side effects.
 *
 * @example
 * Using the hook in VerticalGalleryView:
 * \`\`\`typescript
 * export function VerticalGalleryView(props: Props): JSXElement {
 *   const [containerRef, setContainerRef] = createSignal<HTMLDivElement | null>(null);
 *   const [isVisible, setIsVisible] = createSignal(true);
 *
 *   // Manage all gallery lifecycle effects
 *   useGalleryLifecycle({
 ...
 * \`\`\`
 *
 * @see {@link UseGalleryLifecycleOptions} - Configuration interface
 * @see {@link ensureGalleryScrollAvailable} from '@edge/dom/ensure-gallery-scroll' - Scroll setup
 * @see {@link animateGalleryEnter} from '@shared/utils/css/css-animations' - Enter animation
 * @see {@link observeViewportCssVars} from '@shared/dom/viewport' - Viewport tracking
 */
 ...
 *
 * **Purpose**
 * Defines the reactive dependencies that trigger lifecycle effects.
 * All properties are accessor functions (getters) to maintain Solid.js
 * fine-grained reactivity without breaking on destructuring.
 *
 * **Properties**
 *
 * 1. **containerEl** - Gallery container reference (main DOM element)
 *    - Type: () => HTMLDivElement | null
 *    - Must return the parent container holding media items
 *    - Used by: scroll setup, animation, video cleanup, viewport tracking
 *    - Can change if container is recreated (e.g., during re-renders)
 *    - null means container not yet mounted or was unmounted
 *
 * 2. **toolbarWrapperEl** - Toolbar wrapper reference (height measurable)
 *    - Type: () => HTMLDivElement | null
 *    - Must return an element with measurable getBoundingClientRect
 *    - Used by: viewport tracking to determine toolbar height
 *    - Toolbar height used to adjust gallery padding/scroll area
 *    - Can be null if toolbar not yet mounted
 *    - Must be stable reference (same element for height calculations)
 *
 * 3. **isVisible** - Gallery visibility state (boolean signal)
 *    - Type: () => boolean
 *    - true when gallery is open and visible to user
 *    - false when gallery is closed or hidden
 *    - Triggers enter animation when becomes true
 *    - Triggers exit animation + video cleanup when becomes false
 *    - Critical dependency for animation timing
 *
 * **Type Safety**
 * All accessors are readonly to prevent accidental modification.
 * Return types are explicit (HTMLDivElement | null, not Element).
 * Signals must be called like `containerEl()` not `containerEl`.
 *
 * **Null Handling**
 * All effects handle null values gracefully:
 * - Container setup: skips if element is null
 * - Animation: skips if container is null
 * - Viewport tracking: skips if container or wrapper is null
 * - No errors thrown on null values
 *
 * **Signal vs Static**
 * Properties can be:
 * - Signals: () => element from createSignal
 * - Derived: () => element from createMemo or property access
 * - Static: () => element from DOM query (less reactive)
 *
 * Examples:
 * \`\`\`typescript
 * // Signal (most reactive)
 * const [containerRef, setContainerRef] = createSignal<HTMLDivElement | null>(null);
 * useGalleryLifecycle({ containerEl: containerRef, ... });
 *
 * // DOM query (static, no reactivity)
 * useGalleryLifecycle({
 *   containerEl: () => document.querySelector('[data-gallery-container]'),
 *   ...
 * });
 *
 * // Derived (conditional)
 * useGalleryLifecycle({
 *   containerEl: () => isOpen() ? document.querySelector(...) : null,
 *   ...
 * });
 * \`\`\`
 *
 * @property containerEl - Gallery container element accessor.
 *   The main container holding media items and scroll area.
 *   Used for scroll setup, animations, video cleanup, viewport tracking.
 *
 * @property toolbarWrapperEl - Toolbar wrapper element accessor.
 *   Measured for height to determine gallery padding/scroll adjustment.
 *   Can be null if toolbar not present.
 *
 * @property isVisible - Visibility state accessor.
 *   true triggers enter animation, false triggers exit + cleanup.
 *
 * @example
 ...
 * const [isVisible, setIsVisible] = createSignal(true);
 *
 * const options: UseGalleryLifecycleOptions = {
 *   containerEl: containerRef,
 *   toolbarWrapperEl: toolbarRef,
 *   isVisible,
 * };
 *
 * useGalleryLifecycle(options);
 * \`\`\`
 */
