 ...
 */
export class UnifiedToastManager {
  private static instance: UnifiedToastManager | null = null;
  ...
  private constructor() {
    ...
    logger.debug('[UnifiedToastManager] 초기화됨');
  }

  /**
   * 싱글톤 인스턴스 가져오기
   */
  public static getInstance(): UnifiedToastManager {
    if (!UnifiedToastManager.instance) {
      UnifiedToastManager.instance = new UnifiedToastManager();
    }
    return UnifiedToastManager.instance;
  }
  ...
  public show(options: ToastOptions): string {
    ...
    logger.debug(`[UnifiedToastManager] Toast 표시: ${options.title} - ${options.message}`);
    return id;
  }
  ...
  public remove(id: string): void {
    ...
    if (filteredToasts.length !== currentToasts.length) {
      ...
      logger.debug(`[UnifiedToastManager] Toast 제거: ${id}`);
    }
  }
  ...
  public clear(): void {
    ...
    logger.debug('[UnifiedToastManager] 모든 Toast 제거');
  }
  ...
  public async init(): Promise<void> {
    logger.debug('[UnifiedToastManager] 서비스 초기화');
  }
  ...
  public cleanup(): void {
    ...
    logger.debug('[UnifiedToastManager] 정리 완료');
  }
  ...
  private notifySubscribers(toasts: ToastItem[]): void {
    this.subscribers.forEach(callback => {
      try {
        ...
        logger.warn('[UnifiedToastManager] 구독자 알림 실패:', error);
      }
    });
  }
  ...
  public static resetInstance(): void {
    if (UnifiedToastManager.instance) {
      UnifiedToastManager.instance.cleanup();
      UnifiedToastManager.instance = null;
      logger.debug('[UnifiedToastManager] 싱글톤 초기화됨');
    }
  }
}
 ...
 */
export const unifiedToastManager = UnifiedToastManager.getInstance();

/**
 * 기존 코드와의 호환성을 위한 별칭
 * 점진적 마이그레이션을 위해 제공
 */
export const toastController = unifiedToastManager;
...
export function addToast(toast: Omit<ToastItem, 'id'>): string {
  ...
  return unifiedToastManager.show(options);
}
...
export const toasts = {
  get value(): ToastItem[] {
    return unifiedToastManager.getToasts();
  },
  set value(_: ToastItem[]) {
    // 직접 설정은 허용하지 않음 - 관리자를 통해서만 변경 가능
    logger.warn(
      '[UnifiedToastManager] 직접 상태 설정은 허용되지 않습니다. 관리자 메서드를 사용하세요.'
    );
  },
  subscribe(callback: (value: ToastItem[]) => void) {
    return unifiedToastManager.subscribe(callback);
  },
};
 ...
 * 마이그레이션 가이드:
 *
 * 기존 ToastController.ts 사용 → UnifiedToastManager로 교체
 * - toastController.show() → unifiedToastManager.show() (또는 기존 toastController 별칭 사용)
 * - toastController.success() → unifiedToastManager.success()
 *
 ...
 * - toasts.value → unifiedToastManager.getToasts() 또는 toasts.value
 * - toasts.subscribe() → unifiedToastManager.subscribe() 또는 toasts.subscribe()
 */
