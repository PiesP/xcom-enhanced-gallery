/**
 * @fileoverview 통합 리소스 매니저 - Core 레이어
 * @version 2.0.0 - Clean Architecture Core 이동 완료
 ...
 * 타이머 리소스
 */
export interface TimerResource extends ResourceEntry {
  type: 'timer';
  timerId: number;
  callback: () => void;
  delay: number;
 ...
 * 인터벌 리소스
 */
export interface IntervalResource extends ResourceEntry {
  type: 'interval';
  intervalId: number;
  callback: () => void;
  interval: number;
 ...
 * 이벤트 리스너 리소스
 */
export interface EventResource extends ResourceEntry {
  type: 'event';
  target: EventTarget;
  event: string;
 ...
 * Observer 리소스
 */
export interface ObserverResource extends ResourceEntry {
  type: 'observer';
 ...
 * AbortController 리소스
 */
export interface ControllerResource extends ResourceEntry {
  type: 'controller';
 ...
 * Object URL 리소스
 */
export interface URLResource extends ResourceEntry {
  type: 'url';
  url: string;
  blob?: Blob;
}
...
/**
 * 메모리 리소스 (일반적인 cleanup 가능한 리소스)
 */
export interface MemoryResource extends ResourceEntry {
  type: 'memory';
  resource: unknown;
  customCleanup: () => void;
}
