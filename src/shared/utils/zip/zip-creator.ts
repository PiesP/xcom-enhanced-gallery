...
import { getFflate } from '../vendors';

// 간단한 로거 구현
const logger = {
  debug: (message: string, ...args: unknown[]) => {
    if (process.env.NODE_ENV === 'development') {
      console.info(`[ZipCreator] ${message}`, ...args);
    }
  },
  error: (message: string, ...args: unknown[]) => {
    console.error(`[ZipCreator] ${message}`, ...args);
  },
};
...
export class ZipCreator {
  ...
  addFile(filename: string, data: Uint8Array): boolean {
    try {
      ...
      // 중복 파일명 처리
      let counter = 1;
      while (this.files.has(finalFilename)) {
        const parts = filename.split('.');
        if (parts.length > 1) {
          const ext = parts.pop();
          const base = parts.join('.');
          finalFilename = this.sanitizeFilename(`${base}(${counter}).${ext}`);
        } else {
          finalFilename = this.sanitizeFilename(`${filename}(${counter})`);
        }
        counter++;
      }
      ...
      this.files.set(finalFilename, entry);
      logger.debug(`Added file: ${finalFilename} (${data.length} bytes)`);
   ...
   * ZIP에 텍스트 파일을 추가합니다
   */
  addTextFile(filename: string, text: string): boolean {
    try {
      const fflate = getFflate();
      const data = fflate.strToU8(text);
      return this.addFile(filename, data);
    } catch (error) {
      logger.error('Failed to add text file:', error);
      throw error;
    }
  }
  ...
  async createZip(
    ...
    try {
      ...
      if (this.files.size === 0) {
        // 빈 ZIP 파일 생성
        const emptyZipData = new Uint8Array([
      ...
      this.files.forEach((entry, filename) => {
  ...
  private validateFilename(filename: string): FileValidationResult {
    ...
    // Control characters 검사 (0x00-0x1F)
    const hasControlChars = filename
      .split('')
      .some(char => char.charCodeAt(0) >= 0 && char.charCodeAt(0) <= 31);
