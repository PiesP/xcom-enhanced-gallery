...
export const galleryDebugUtils = {
  ...
  checkState(): void {
    ...
    const galleryState = (window as any).xegGalleryState;
    if (galleryState) {
  ...
  fullDiagnosis(): void {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const preactSignals = (window as any).preactSignals;
...
if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (window as any).galleryDebug = galleryDebugUtils;
