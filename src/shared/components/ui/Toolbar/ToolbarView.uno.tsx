/**
 * @fileoverview UnoCSS Toolbar View Component
 * @version 1.0.0 - CSS Modules to UnoCSS Migration
 * @description Gallery toolbar UI using UnoCSS atomic classes
 * @module @shared/components/ui/Toolbar
 *
 * Features:
 * - Navigation controls (prev/next)
 * - Media counter with progress bar
 * - Fit mode buttons (original, width, height, container)
 * - Download buttons (current, all)
 * - Settings panel with theme/language controls
 * - Tweet text panel
 * - UnoCSS atomic classes for optimal bundle size
 *
 * @example
 * ```tsx
 * import { ToolbarView } from '@shared/components/ui/Toolbar/ToolbarView.uno';
 *
 * <ToolbarView {...toolbarViewProps} />
 * ```
 */
...
/**
 * UnoCSS Toolbar View Component
 *
 * Gallery toolbar UI with navigation, fit modes, downloads, and settings.
 * Uses atomic CSS classes for optimal styling.
 *
 * @param props - Toolbar view configuration
...
export function ToolbarView(props: ToolbarViewProps): JSXElement {
  ...
  const toolbarButtonClass = (...extra: Array<string | undefined>) =>
    createClassName(
      ...
      'xeg-toolbar-button-focus',
      'xeg-inline-center',
      ...extra
    );

  const toolbarRootClass = () => {
    const isSettingsExpanded = props.settingsController.isSettingsExpanded();
    const isTweetExpanded = props.isTweetPanelExpanded();

    return createClassName(
      'xeg-toolbar-root',
      isSettingsExpanded || isTweetExpanded ? 'xeg-toolbar-root-expanded' : undefined
    );
  };
  ...
  const settingsPanelClass = () =>
    createClassName(
      'xeg-toolbar-panel',
      'xeg-toolbar-settings-panel',
      props.settingsController.isSettingsExpanded() ? 'xeg-toolbar-panel-expanded' : undefined
    );

  const tweetPanelClass = () =>
    createClassName(
      'xeg-toolbar-panel',
      'xeg-toolbar-tweet-panel',
      props.isTweetPanelExpanded() ? 'xeg-toolbar-panel-expanded' : undefined
    );
  ...
  return (
    ...
    >
      {/* Toolbar Content */}
      <div class="xeg-toolbar-content xeg-row-center" data-gallery-element="toolbar-content">
        <div class="xeg-toolbar-controls" data-gallery-element="toolbar-controls">
          {/* Navigation: Previous */}
          <IconButton
          ...
          {/* Media Counter */}
          <div class="xeg-toolbar-counter-block" data-gallery-element="counter-section">
            <div class="xeg-toolbar-counter-wrapper xeg-inline-center">
              <span
                ref={setCounterElement}
                class="xeg-toolbar-counter xeg-inline-center"
                aria-live="polite"
                data-gallery-element="counter"
                data-focused-index={displayedIndex()}
                data-current-index={currentIndex()}
              >
                <span class="xeg-toolbar-counter-current">{displayedIndex() + 1}</span>
                <span class="xeg-toolbar-counter-separator">/</span>
                <span class="xeg-toolbar-counter-total">{totalCount()}</span>
              </span>
              <div class="xeg-toolbar-progress">
                <div class="xeg-toolbar-progress-fill" style={{ width: props.progressWidth() }} />
              </div>
            </div>
          </div>
          ...
          {props.fitModeOrder.map(({ mode, Icon }) => {
            ...
            return (
              <IconButton
                class={toolbarButtonClass(
                  'xeg-toolbar-fit-button',
                  activeFitMode() === mode ? 'xeg-toolbar-fit-button-selected' : undefined
                )}
                size="toolbar"
          ...
          {/* Download Current */}
          <IconButton
            class={toolbarButtonClass('xeg-toolbar-download-button', 'xeg-toolbar-button-primary')}
            size="toolbar"
          ...
          {canDownloadAll() && (
            <IconButton
              class={toolbarButtonClass(
                'xeg-toolbar-download-button',
                'xeg-toolbar-button-success'
              )}
              size="toolbar"
          ...
          {/* Close Button */}
          <IconButton
            class={toolbarButtonClass('xeg-toolbar-button-danger')}
            size="toolbar"
            intent="danger"
      ...
      >
        <Show when={props.isTweetPanelExpanded() && hasTweetContent()}>
          <Suspense fallback={<div class="xeg-toolbar-tweet-body">Loading...</div>}>
            <TweetTextPanelLazy
