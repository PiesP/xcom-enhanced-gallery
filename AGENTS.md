...
- **책임**: 모든 코드 변경사항에 대한 **전체 검증** (다른 워크플로우의 기반)
- Node 22 단일 환경에서 다음을 수행:
  - **품질 검증** (표준 명령 사용):
    - Type check: `npx tsgo --project ./tsconfig.json --noEmit`
    - Lint JavaScript:
      `npx eslint ./src --report-unused-disable-directives --max-warnings 0`
    - Lint CSS: `npx stylelint "src/**/*.css"`
    - Lint Markdown:
      `npx markdownlint-cli2 "**/*.md" "#node_modules" "#coverage"`
    - Format check: `npx prettier --check .`
    - Dependency check: `npx depcruise src --validate`
    - CodeQL Analysis: `github/codeql-action` (GitHub 공식 Action,
      security-extended 쿼리)
  - **단위 테스트**: JSDOM 기반 (커버리지 포함)
  - **브라우저 테스트**: `npx vitest --project browser run` (2개 샤드 병렬 실행)
  - **E2E 테스트**: `npx playwright test playwright/smoke` (Chromium)
  - **접근성 테스트**: `npx playwright test playwright/accessibility` (axe-core
    WCAG 2.1 AA)
  - **빌드 검증**: `npx vite build` (dev/prod) +
    `node ./scripts/validate-build.js` (프로젝트 특화)
  - 커버리지/빌드/E2E 실패 아티팩트 업로드
- **표준화 원칙** (2025-10-29):
  - ✅ 커스텀 스크립트 최소화: 표준 도구 직접 호출 (npx) 우선
  - ✅ 투명성: 각 단계가 GitHub Actions UI에 명확히 표시
  - ✅ 디버깅: 실패 지점 즉시 파악 (annotations 활용)
  - ✅ npm ci 플래그 통일: 모든 워크플로우에서
    `--prefer-offline --no-audit --legacy-peer-deps` 사용
  - ✅ NODE_OPTIONS 제거: .npmrc 전역 설정 우선 (8192MB)
  - ✅ CodeQL: GitHub 공식 Action 사용 (CI), 로컬은 커스텀 스크립트 유지
  - 🔒 프로젝트 특화 스크립트 유지: validate-build.js, maintenance-check.js
    (로컬용 check-codeql.js)
- **워크플로우 책임 분리** (2025-10-29):
  - ci.yml: 전체 검증 (품질, 테스트, E2E, 빌드) - **다른 워크플로우의 기반**
  - release.yml: 릴리즈 산출물 생성 (smoke + prod build + release) - **ci.yml
    검증 가정**
  - security.yml: 보안 전용 (npm audit, license) - **주간 스케줄**
  - maintenance.yml: 유지보수 전용 (월간 점검, 이슈 생성) - **월간 스케줄**
- **성능 최적화** (Phase 1.1):
...
- **책임**: 보안 전용 모니터링 (주간 스케줄 + 온디맨드)
- `npm audit`와 라이선스 보고서 업로드를 자동화
- CI와 분리하여 보안 알림이 PR을 차단하지 않도록 함

CodeQL (보안 정적 분석)

- **CI 환경 (필수)**: `github/codeql-action@v3` (GitHub Advanced Security)
  - 자동 실행: 모든 PR/커밋
  - 쿼리 스위트: `security-extended` (XSS, 인젝션, prototype pollution 등)
  - 결과: GitHub Security tab (Code scanning alerts)
- **로컬 환경 (선택)**: `node scripts/check-codeql.js`
  - 지원 도구: `gh codeql` (GitHub CLI 확장) 또는 `codeql` (직접 설치 CLI)
  - 쿼리 팩 자동 다운로드: `codeql/javascript-queries`
  - 출력 옵션: 기본/JSON/마크다운 리포트
- 성능 최적화:
  - 도구 감지 결과 캐싱 (0.3초 절약)
...
- master로의 버전 변경(또는 수동 트리거) 시:
  - **품질 검증** (표준 명령 사용): CI와 동일한 표준 명령 실행
  - **스모크 테스트**: `npx vitest --project smoke run`
