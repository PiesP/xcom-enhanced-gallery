...
(function() {
	...
	*/
	var ko = buildLanguageStringsFromValues([
		"이전",
		"다음",
		"다운로드",
	...
	*/
	...
	var MEDIA_URL_POLICY = {
		allowedProtocols: Object.freeze(new Set([
			"http:",
			"https:",
			"blob:"
		])),
	...
	*/
	var KEYBOARD_HELP_TIMEOUT_MS = 6e3;
	...
	var SharedObserver = {
		observe(element, callback, options = {}) {
			...
			return () => {
				...
				if (count <= 0) poolEntry.refCount.delete(element);
				else {
		...
		unobserve(element) {
			for (const [, poolEntry] of observerPool) {
				...
				poolEntry.observer.unobserve(element);
				if (poolEntry.refCount.size === 0) poolEntry.observer.disconnect();
			}
			for (const [key, poolEntry] of observerPool) if (poolEntry.refCount.size === 0) observerPool.delete(key);
		}
	};
	//#endregion
	...
	*/
	...
	var _tmpl$$3 = /* @__PURE__ */ template(`<div data-gallery-element=toolbar><div><div><div><div><span aria-live=polite><span></span><span>/</span><span></span></span><div role=progressbar aria-valuemin=1><div></div></div></div></div></div></div><div id=toolbar-settings-panel data-gallery-scrollable=true role=region aria-label="Settings Panel"aria-labelledby=settings-button data-gallery-element=settings-panel></div><div id=toolbar-tweet-panel role=region aria-labelledby=tweet-text-button data-gallery-element=tweet-panel>`);
		...
		*/
		...
		return (() => {
			...
			createRenderEffect((_p$) => {
				...
				_v$15 !== _p$.y && setAttribute(_el$0, "aria-valuenow", _p$.y = _v$15);
				_v$16 !== _p$.g && setAttribute(_el$0, "aria-valuemax", _p$.g = _v$16);
				_v$17 !== _p$.p && setAttribute(_el$0, "aria-label", _p$.p = _v$17);
				...
				_v$21 !== _p$.O && className(_el$11, _p$.O = _v$21);
				_v$22 !== _p$.I && setAttribute(_el$11, "aria-label", _p$.I = _v$22);
				return _p$;
			}, {
				e: void 0,
				t: void 0,
		...
		const handleDownload = async (type) => {
			...
			try {
				...
				if (type === "current") {
					...
					} else notifyError(languageService.translate("msg.dl.one.err.t"), "No media item selected. Please re-open the gallery and try again.");
				} else {
	...
	*/
	var _tmpl$ = /* @__PURE__ */ template(`<div aria-live=polite data-xeg-error-boundary role=alert><p class=xeg-error-boundary__title></p><p class=xeg-error-boundary__body></p><button class=xeg-error-boundary__action type=button>`);
		...
		return [createComponent(Show, {
			...
			children: (error) => {
				...
				return (() => {
					...
					createRenderEffect(() => _el$4.disabled = retryCount() >= MAX_RETRIES);
					return _el$;
				})();
			}
		})];
