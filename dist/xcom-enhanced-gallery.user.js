...
var XcomEnhancedGallery = (function(exports) {
	...
	function createMergedAbortController(signals) {
		...
		const onAbort = () => {
			for (const sig of signals) if (sig.aborted) {
	...
	*/
	function mergeAbortSignalsWithCleanup(signals) {
	...
	//#endregion
	//#region node_modules/.pnpm/@piesp+browser-core@file+packages+core/node_modules/@piesp/browser-core/src/error/abort-signal.ts
	/**
	...
	*/
	function createAbortError(message = "The operation was aborted.", cause) {
	...
	*/
	...
	//#endregion
	//#region node_modules/.pnpm/@piesp+browser-core@file+packages+core/node_modules/@piesp/browser-core/src/util/guards.ts
	/**
	...
	*/
	function isHTMLElement(element) {
		if (typeof element !== "object" || element === null || typeof HTMLElement === "undefined") return false;
		const titleGetter = Object.getOwnPropertyDescriptor(HTMLElement.prototype, "title")?.get;
	...
	var log = createLogger("Notification");
	...
	/** Maximum total bytes in the demand-driven download cache (100 MB). */
	var DOWNLOAD_CACHE_MAX_BYTES = 104857600;
	/**
	...
	*/
	...
	function getUserscript() {
		...
		cachedUserscriptAPI = {
			async download(url, filename, signal) {
				...
				if (gmDownloadModern) return new Promise((resolve, reject) => {
					...
					try {
						const handle = gmDownloadModern({
							...
							onload: complete,
							onerror: fail,
							ontimeout: () => fail(/* @__PURE__ */ new Error("GM_download timed out"))
						});
						if (settled) return;
						if (signal?.aborted) {
							handle.abort();
	...
	*/
	var ERROR_NOTIFICATION_TIMEOUT_MS = 5e3;
	...
	*/
	var getDownloadAdapter = createAdapter(() => new MV3DownloadAdapter(), () => {
		...
		return {
			download: (url, filename, _headers, signal) => api.download(url, filename, signal),
			downloadBlob: (blob, filename, signal) => api.downloadBlob(blob, filename, signal),
	...
	*/
	...
	function ensureErrorMessage(message, fallback = UNKNOWN_ERROR_MESSAGE) {
		const trimmed = message.trim();
		return trimmed.length > 0 ? trimmed : fallback;
	}
	function normalizeErrorMessage(error) {
		try {
			if (error instanceof Error) return ensureErrorMessage(error.message, ensureErrorMessage(error.name, "Error"));
	...
	function createReporter(context) {
		const report = (severity) => (error, options) => {
			...
			if (severity === "info") {} else if (severity === "warning") logger.warn(message, payload);
	...
	*/
	var LANGUAGE_CODES = LOCALE_CODES;
	var NORMALIZED_LANGUAGE_CODES = new Set(LANGUAGE_CODES.map((code) => code.toLowerCase()));
	/**
	...
	*/
	...
	function decodeValue(raw) {
		...
		if (isRecord(parsed) && parsed.__xegStorageEnvelope === STORAGE_ENVELOPE_VERSION && typeof parsed.updatedAt === "number" && Object.hasOwn(parsed, "value")) return {
			value: parsed.value,
	...
	*/
	...
	//#endregion
	//#region node_modules/.pnpm/@piesp+browser-core@file+packages+core/node_modules/@piesp/browser-core/src/async/deferred.ts
	/**
	...
	*/
	...
	//#endregion
	//#region src/shared/services/media/download-media-cache.ts
	...
	*/
	var DownloadMediaCache = class {
		...
		head = null;
		tail = null;
		constructor(maxEntries = DEFAULT_CACHE_MAX_ENTRIES, maxBytes = 104857600) {
			this.maxEntries = maxEntries;
			this.maxBytes = maxBytes;
		}
		getOrFetch(media, signal) {
		...
		fetchAndCache(url, callerSignal) {
			const controller = new AbortController();
			const signalScope = callerSignal ? mergeAbortSignalsWithCleanup([controller.signal, callerSignal]) : {
				signal: controller.signal,
				cleanup: () => void 0
			};
			...
			let cachePromise;
			cachePromise = getHttpRequestService().get(url, {
	...
	*/
	...
	function extractMediaUrlCandidatesFromElement(element) {
		if (element instanceof HTMLImageElement) {
			const attr = element.getAttribute("src");
			return collectUniqueTruthy([
	...
	function collectUniqueTruthy(values) {
		const result = [];
	...
	*/
	...
	var STATUS_PATH_PATTERN = /\/status\/(\d+)(?:\/|$)/u;
	var parseTrustedStatusLink = (inputUrl) => {
		try {
			const url = new URL(inputUrl, DEFAULT_TWEET_ORIGIN);
			if (!isHostMatching(url, TWITTER_HOSTS, { allowSubdomains: true }) || url.protocol !== "https:" && url.protocol !== "http:") return null;
			const tweetId = url.pathname.match(STATUS_PATH_PATTERN)?.[1];
	...
	*/
	...
	function determineClickedIndex(clickedElement, mediaItems) {
		try {
			const normalizedElementUrls = resolveClickedElementUrls(clickedElement).map((url) => normalizeMediaUrl(url)).filter((url) => !!url);
			...
			const index = mediaItems.findIndex((item) => {
				...
				return getNormalizedMediaCandidates(item).some((candidate) => clickedCandidates.has(candidate));
			});
	...
	function resolveClickedElementUrls(clickedElement) {
		...
		const backgroundUrl = extractBackgroundImageUrl(mediaElement ?? clickedElement, 3);
	...
	*/
	...
	var MediaService = class {
		...
		downloadCache = new DownloadMediaCache(20, DOWNLOAD_CACHE_MAX_BYTES);
		didCleanup = false;
		_initialized = false;
		/** Initialize service (idempotent) */
		async initialize() {
			...
			this.downloadCache ??= new DownloadMediaCache(20, DOWNLOAD_CACHE_MAX_BYTES);
		...
		getDownloadMedia(media, signal) {
			return this.downloadCache?.getOrFetch(media, signal) ?? null;
		}
		cancelPendingMediaRequests() {
	...
	*/
	...
	//#endregion
	//#region src/shared/state/signals/gallery-navigation.ts
	function resolveAdjacentNavigationTarget(anchorIndex, direction, itemCount) {
	...
	*/
	...
	function commitNavigation(targetIndex, source) {
		batch(() => {
	...
	function handleNavigation(key) {
		...
		switch (key) {
			...
			case "PageUp":
				...
				break;
			case "PageDown": navigateToItem(Math.min(total - 1, current + 5), "keyboard");
		}
	}
	/**
	...
	*/
	function handleVideoControl(key) {
		switch (key) {
			case " ":
				if (keyboardDebouncer.shouldExecute(" ", 150)) executeVideoControl("togglePlayPause");
				break;
			case "ArrowUp":
			...
			case "ArrowDown":
				...
				break;
			case "m":
			case "M": if (keyboardDebouncer.shouldExecute("M", 100)) executeVideoControl("toggleMute");
		}
	}
	/**
	...
	*/
	var GalleryApp = class {
		...
		renderer;
		mediaClickCounter = 0;
		mediaClickAbortController = null;
		sessionEpoch = 0;
		...
		async handleMediaClick(element, _event) {
			...
			const signalScope = sessionSignal ? mergeAbortSignalsWithCleanup([sessionSignal, clickController.signal]) : {
				signal: clickController.signal,
				cleanup: () => void 0
			};
			try {
				const result = await getMediaService().extractFromClickedElement(element, { signal: signalScope.signal });
				...
				signalScope.cleanup();
				if (this.mediaClickAbortController === clickController) this.mediaClickAbortController = null;
			}
		}
		isCurrentMediaClick(opId, epoch, controller) {
			return opId === this.mediaClickCounter && epoch === this.sessionEpoch && this.initialized && this.mediaClickAbortController === controller && !controller.signal.aborted;
		}
	...
	*/
	function useGalleryScrollCorrection(options) {
		...
		const debouncedScrollCorrection = debounce((index, mediaId) => {
			if (!isVisible()) return;
	...
	*/
	...
	//#endregion
	//#region src/features/gallery/logic/focus-candidate-selection.ts
	/** Rank premeasured visible items without reading or mutating DOM state. */
	function selectBestFocusCandidate(viewportRect, itemRects) {
		...
		for (const itemRect of itemRects) {
			...
			const visibleHeight = Math.max(0, Math.min(itemBottom, viewportBottom) - Math.max(itemRect.top, viewportTop));
			...
			if (topDistance <= 50 && visibilityRatio > .1 && (!topAlignedCandidate || topDistance < topAlignedCandidate.distance)) topAlignedCandidate = {
	...
	var SharedObserver = {
		observe(element, callback, options = {}) {
				...
				const observer = new IntersectionObserver((entries) => {
					for (const e of entries) {
						if (!e.target.isConnected) {
							...
							if (callbacks.size === 0) {
								observer.disconnect();
								if (observerPool.get(key)?.observer === observer) observerPool.delete(key);
							}
							continue;
						}
						const targetCallbacks = callbacks.get(e.target);
						if (!targetCallbacks) continue;
						for (const cb of [...targetCallbacks.values()]) try {
							cb(e);
						} catch (error) {}
					}
				}, options);
			...
			if (!targetCallbacks) {
				targetCallbacks = /* @__PURE__ */ new Map();
		...
		*/
		gc() {
			...
			for (const [key, poolEntry] of observerPool) {
				const stale = [];
				for (const element of poolEntry.callbacks.keys()) if (!element.isConnected) stale.push(element);
	...
	*/
	function useGalleryItemScroll(containerRef, currentIndex, totalItems, options = {}) {
		...
		const getCachedItem = (index, itemsRoot) => {
			...
			const element = itemsRoot.querySelectorAll("[data-gallery-element=\"item\"]")[index];
	...
	*/
	function ensureGalleryScrollAvailable(element) {
		if (!element) return;
		element.querySelectorAll("[data-gallery-element=\"items\"]").forEach((el) => {
	...
	*/
	function useVideoVolumePersistence(options) {
		...
		const debouncedSaveVolume = debounce((volume, muted) => {
	...
	*/
	var _tmpl$$11 = /*#__PURE__*/ template(`<li data-gallery-element=item><div>`);
	var _tmpl$2$6 = /*#__PURE__*/ template(`<div><div role=status aria-live=polite>`);
	var _tmpl$3$2 = /*#__PURE__*/ template(`<video controls>`);
	var _tmpl$4$1 = /*#__PURE__*/ template(`<button type=button><img decoding=async>`, true, false, false);
		...
		return (() => {
			...
			insert(_el$2, (() => {
				...
				return () => _c$() && (() => {
					...
					createRenderEffect((_p$) => {
						var _v$1 = VerticalImageItem_module_default.placeholder, _v$10 = cx("xeg-spinner", VerticalImageItem_module_default.loadingSpinner), _v$11 = translate("msg.gal.loading");
						_v$1 !== _p$.e && className(_el$3, _p$.e = _v$1);
						_v$10 !== _p$.t && className(_el$4, _p$.t = _v$10);
						_v$11 !== _p$.a && setAttribute(_el$4, "aria-label", _p$.a = _v$11);
						return _p$;
					}, {
						e: void 0,
						t: void 0,
			...
			insert(_el$2, (() => {
				...
				return () => _c$2() ? (() => {
					...
					addEventListener(_el$5, "focus", local.onFocus);
					use(setVideoRef, _el$5);
					createRenderEffect((_p$) => {
						var _v$12 = displaySrc(), _v$13 = cx(VerticalImageItem_module_default.video, fitModeClass(), isLoaded() ? VerticalImageItem_module_default.loaded : VerticalImageItem_module_default.loading), _v$14 = local["aria-label"] || imageAltText(), _v$15 = local["aria-describedby"], _v$16 = isFocused() ? 0 : -1;
						_v$12 !== _p$.e && setAttribute(_el$5, "src", _p$.e = _v$12);
						_v$13 !== _p$.t && className(_el$5, _p$.t = _v$13);
						_v$14 !== _p$.a && setAttribute(_el$5, "aria-label", _p$.a = _v$14);
						_v$15 !== _p$.o && setAttribute(_el$5, "aria-describedby", _p$.o = _v$15);
						_v$16 !== _p$.i && setAttribute(_el$5, "tabindex", _p$.i = _v$16);
						return _p$;
					}, {
						e: void 0,
						t: void 0,
					...
					_el$6.$$click = handleImageClick;
					addEventListener(_el$6, "focus", local.onFocus);
					_el$7.addEventListener("dragstart", preventDragStart);
					_el$7.addEventListener("error", handleMediaError);
					_el$7.addEventListener("load", handleMediaLoad);
					use(setImageRef, _el$7);
					createRenderEffect((_p$) => {
						var _v$17 = VerticalImageItem_module_default.mediaAction, _v$18 = local["aria-label"] || imageAltText(), _v$19 = local["aria-describedby"], _v$20 = isFocused() ? 0 : -1, _v$21 = displaySrc(), _v$22 = imageAltText(), _v$23 = dimensions().width, _v$24 = dimensions().height, _v$25 = shouldEagerLoad() ? "eager" : "lazy", _v$26 = cx(VerticalImageItem_module_default.image, fitModeClass(), isLoaded() ? VerticalImageItem_module_default.loaded : VerticalImageItem_module_default.loading);
						_v$17 !== _p$.e && className(_el$6, _p$.e = _v$17);
						_v$18 !== _p$.t && setAttribute(_el$6, "aria-label", _p$.t = _v$18);
						_v$19 !== _p$.a && setAttribute(_el$6, "aria-describedby", _p$.a = _v$19);
						_v$20 !== _p$.o && setAttribute(_el$6, "tabindex", _p$.o = _v$20);
						_v$21 !== _p$.i && setAttribute(_el$7, "src", _p$.i = _v$21);
						_v$22 !== _p$.n && setAttribute(_el$7, "alt", _p$.n = _v$22);
						_v$23 !== _p$.s && setAttribute(_el$7, "width", _p$.s = _v$23);
						_v$24 !== _p$.h && setAttribute(_el$7, "height", _p$.h = _v$24);
						_v$25 !== _p$.r && setAttribute(_el$7, "loading", _p$.r = _v$25);
						_v$26 !== _p$.d && className(_el$7, _p$.d = _v$26);
						return _p$;
					}, {
						e: void 0,
						t: void 0,
			...
			insert(_el$2, (() => {
				...
				return () => _c$3() && (() => {
					...
					insert(_el$0, () => translate("msg.gal.loadFail", { type: isVideo() ? "video" : "image" }));
					createRenderEffect((_p$) => {
						var _v$27 = VerticalImageItem_module_default.error, _v$28 = VerticalImageItem_module_default.errorIcon, _v$29 = VerticalImageItem_module_default.errorText;
						_v$27 !== _p$.e && className(_el$8, _p$.e = _v$27);
						_v$28 !== _p$.t && className(_el$9, _p$.t = _v$28);
						_v$29 !== _p$.a && className(_el$0, _p$.a = _v$29);
						return _p$;
					}, {
						e: void 0,
						t: void 0,
			...
			createRenderEffect((_p$) => {
				var _v$ = containerClasses(), _v$2 = local.index, _v$3 = resolvedFitMode(), _v$4 = isLoaded() ? "true" : "false", _v$5 = hasIntrinsicSize() ? "true" : void 0, _v$6 = mergedStyle(), _v$7 = local.index + 1, _v$8 = totalItems(), _v$9 = void 0, _v$0 = VerticalImageItem_module_default.imageWrapper;
				...
				_v$7 !== _p$.s && setAttribute(_el$, "aria-posinset", _p$.s = _v$7);
				_v$8 !== _p$.h && setAttribute(_el$, "aria-setsize", _p$.h = _v$8);
				_v$9 !== _p$.r && setAttribute(_el$, "data-testid", _p$.r = _v$9);
				_v$0 !== _p$.d && className(_el$2, _p$.d = _v$0);
				return _p$;
			}, {
				e: void 0,
				t: void 0,
	...
	//#endregion
	//#region src/constants/setting-options.ts
	...
	function isImageFitMode(value) {
		return isStringOption(value, IMAGE_FIT_MODES);
	}
	function isVideoClickMode(value) {
	...
	*/
	function computePosition(triggerRect, preferredPlacement, offset) {
		...
		return {
			x: Math.max(104, Math.min(centerX, viewportWidth - VIEWPORT_PADDING - estimatedHalfWidth)),
			y,
			actualPlacement
		};
	}
	...
	*/
	function Tooltip(props) {
		...
		const resolveTriggerElement = (event) => {
			if (event?.target instanceof HTMLElement && event.target !== triggerRef) return event.target.closest("button, select, input, textarea, a[href], [tabindex]");
			const firstChild = triggerRef.firstElementChild;
			return firstChild instanceof HTMLElement ? firstChild : null;
		};
		const attachDescription = (element) => {
			if (describedElement === element) return;
			if (describedElement) if (previousDescribedBy === null) describedElement.removeAttribute("aria-describedby");
		...
		return (() => {
			...
			_el$.$$focusin = (event) => show(event);
			_el$.addEventListener("mouseleave", hide);
			_el$.addEventListener("mouseenter", (event) => show(event));
			var _ref$ = triggerRef;
			...
			insert(_el$, (() => {
				...
				return () => _c$() && createComponent(Portal, {
					...
					get children() {
						...
						_el$2.addEventListener("mouseleave", hide);
						_el$2.addEventListener("mouseenter", keepVisible);
						setAttribute(_el$2, "id", tooltipId);
						insert(_el$2, () => local.content, null);
						createRenderEffect((_p$) => {
							var _v$ = tooltipClass(), _v$2 = tooltipStyle(), _v$3 = arrowClass(), _v$4 = arrowInnerClass();
			...
			createRenderEffect(() => className(_el$, Tooltip_module_default.trigger));
			return _el$;
		})();
	}
	delegateEvents([
	...
	*/
	var _tmpl$$6 = /*#__PURE__*/ template(`<select autocomplete=off required>`);
		...
		return (() => {
			...
			insert(_el$2, createComponent(Tooltip, {
				...
				get children() {
					...
					createRenderEffect((_p$) => {
						var _v$ = local["data-testid"] ? `${local["data-testid"]}-theme-select` : "settings-theme-select", _v$2 = cx("xeg-inline-center", SettingsControls_module_default.select), _v$3 = strings().theme.title, _v$4 = local["data-testid"] ? `${local["data-testid"]}-theme-label` : "settings-theme-label", _v$5 = void 0;
						...
						_v$5 !== _p$.i && setAttribute(_el$4, "data-testid", _p$.i = _v$5);
						return _p$;
					}, {
						e: void 0,
						t: void 0,
			...
			insert(_el$5, createComponent(Tooltip, {
				...
				get children() {
					...
					createRenderEffect((_p$) => {
						var _v$6 = local["data-testid"] ? `${local["data-testid"]}-language-select` : "settings-language-select", _v$7 = cx("xeg-inline-center", SettingsControls_module_default.select), _v$8 = strings().language.title, _v$9 = local["data-testid"] ? `${local["data-testid"]}-language-label` : "settings-language-label", _v$0 = void 0;
						_v$6 !== _p$.e && setAttribute(_el$7, "id", _p$.e = _v$6);
						_v$7 !== _p$.t && className(_el$7, _p$.t = _v$7);
						_v$8 !== _p$.a && setAttribute(_el$7, "aria-label", _p$.a = _v$8);
						_v$9 !== _p$.o && setAttribute(_el$7, "aria-labelledby", _p$.o = _v$9);
						_v$0 !== _p$.i && setAttribute(_el$7, "data-testid", _p$.i = _v$0);
						return _p$;
					}, {
						e: void 0,
						t: void 0,
			...
			createRenderEffect((_p$) => {
				var _v$1 = cx(SettingsControls_module_default.body, local.compact && SettingsControls_module_default.bodyCompact), _v$10 = void 0, _v$11 = cx(SettingsControls_module_default.setting, local.compact && SettingsControls_module_default.settingCompact), _v$12 = local["data-testid"] ? `${local["data-testid"]}-theme-label` : "settings-theme-label", _v$13 = local["data-testid"] ? `${local["data-testid"]}-theme-select` : "settings-theme-select", _v$14 = cx(SettingsControls_module_default.label, local.compact && SettingsControls_module_default.compactLabel), _v$15 = cx(SettingsControls_module_default.setting, local.compact && SettingsControls_module_default.settingCompact), _v$16 = local["data-testid"] ? `${local["data-testid"]}-language-label` : "settings-language-label", _v$17 = local["data-testid"] ? `${local["data-testid"]}-language-select` : "settings-language-select", _v$18 = cx(SettingsControls_module_default.label, local.compact && SettingsControls_module_default.compactLabel);
				_v$1 !== _p$.e && className(_el$, _p$.e = _v$1);
				_v$10 !== _p$.t && setAttribute(_el$, "data-testid", _p$.t = _v$10);
				_v$11 !== _p$.a && className(_el$2, _p$.a = _v$11);
				_v$12 !== _p$.o && setAttribute(_el$3, "id", _p$.o = _v$12);
				_v$13 !== _p$.i && setAttribute(_el$3, "for", _p$.i = _v$13);
				_v$14 !== _p$.n && className(_el$3, _p$.n = _v$14);
				_v$15 !== _p$.s && className(_el$5, _p$.s = _v$15);
				_v$16 !== _p$.h && setAttribute(_el$6, "id", _p$.h = _v$16);
				_v$17 !== _p$.r && setAttribute(_el$6, "for", _p$.r = _v$17);
				_v$18 !== _p$.d && className(_el$6, _p$.d = _v$18);
				return _p$;
			}, {
				e: void 0,
				t: void 0,
	...
	*/
	...
	var _tmpl$$4 = /*#__PURE__*/ template(`<fieldset data-gallery-element=toolbar><legend class=xeg-sr-only></legend><div><div><div><div><span id=xeg-toolbar-counter aria-live=polite><span></span><span>/</span><span></span></span><div role=progressbar aria-valuemin=1 aria-labelledby=xeg-toolbar-counter><div></div></div></div></div></div></div><section id=toolbar-settings-panel data-gallery-scrollable=true aria-labelledby=settings-button data-gallery-element=settings-panel></section><section id=toolbar-tweet-panel aria-labelledby=tweet-text-button data-gallery-element=tweet-panel>`);
	...
	function ToolbarView(props) {
		...
		*/
		...
		return (() => {
			var _el$ = _tmpl$$4(), _el$2 = _el$.firstChild, _el$3 = _el$2.nextSibling, _el$4 = _el$3.firstChild, _el$5 = _el$4.firstChild, _el$6 = _el$5.firstChild, _el$7 = _el$6.firstChild, _el$8 = _el$7.firstChild, _el$9 = _el$8.nextSibling, _el$0 = _el$9.nextSibling, _el$1 = _el$7.nextSibling, _el$10 = _el$1.firstChild, _el$11 = _el$3.nextSibling, _el$12 = _el$11.nextSibling;
			...
			insert(_el$2, () => local["aria-label"] ?? translate("tb.galleryToolbar"));
			...
			insert(_el$8, () => local.displayedIndex() + 1);
			insert(_el$0, totalCount);
			insert(_el$4, () => local.fitModeOrder.map(({ mode, iconName }) => {
			...
			addEventListener(_el$11, "click", local.settingsController.handlePanelClick, true);
			addEventListener(_el$11, "mousedown", local.settingsController.handlePanelMouseDown, true);
			use(assignSettingsPanelRef, _el$11);
			...
			createRenderEffect((_p$) => {
				var _v$ = cx(local.toolbarClass(), toolbarStateClass(), local.settingsController.isSettingsExpanded() ? Toolbar_module_default.settingsExpanded : void 0, local.isTweetPanelExpanded() ? Toolbar_module_default.tweetPanelExpanded : void 0), _v$2 = local["aria-describedby"], _v$3 = isToolbarDisabled(), _v$4 = void 0, _v$5 = local.tabIndex, _v$6 = cx(Toolbar_module_default.toolbarContent, "xeg-row-center"), _v$7 = Toolbar_module_default.toolbarControls, _v$8 = Toolbar_module_default.counterBlock, _v$9 = cx(Toolbar_module_default.mediaCounterWrapper, "xeg-inline-center"), _v$0 = cx(Toolbar_module_default.mediaCounter, "xeg-inline-center"), _v$1 = Toolbar_module_default.currentIndex, _v$10 = Toolbar_module_default.separator, _v$11 = Toolbar_module_default.totalCount, _v$12 = Toolbar_module_default.progressBar, _v$13 = translate("tb.progress"), _v$14 = local.displayedIndex() + 1, _v$15 = totalCount(), _v$16 = Toolbar_module_default.progressFill, _v$17 = local.progressWidth(), _v$18 = cx(Toolbar_module_default.settingsPanel, local.settingsController.isSettingsExpanded() ? Toolbar_module_default.panelExpanded : void 0), _v$19 = translate("tb.settingsPanel"), _v$20 = cx(Toolbar_module_default.tweetPanel, local.isTweetPanelExpanded() ? Toolbar_module_default.panelExpanded : void 0), _v$21 = translate("tb.twPanel");
				_v$ !== _p$.e && className(_el$, _p$.e = _v$);
				_v$2 !== _p$.t && setAttribute(_el$, "aria-describedby", _p$.t = _v$2);
				_v$3 !== _p$.a && (_el$.disabled = _p$.a = _v$3);
				_v$4 !== _p$.o && setAttribute(_el$, "data-testid", _p$.o = _v$4);
				...
				_v$1 !== _p$.l && className(_el$8, _p$.l = _v$1);
				_v$10 !== _p$.u && className(_el$9, _p$.u = _v$10);
				_v$11 !== _p$.c && className(_el$0, _p$.c = _v$11);
				_v$12 !== _p$.w && className(_el$1, _p$.w = _v$12);
				_v$13 !== _p$.m && setAttribute(_el$1, "aria-label", _p$.m = _v$13);
				_v$14 !== _p$.f && setAttribute(_el$1, "aria-valuenow", _p$.f = _v$14);
				_v$15 !== _p$.y && setAttribute(_el$1, "aria-valuemax", _p$.y = _v$15);
				_v$16 !== _p$.g && className(_el$10, _p$.g = _v$16);
				_v$17 !== _p$.p && setStyleProperty(_el$10, "width", _p$.p = _v$17);
				_v$18 !== _p$.b && className(_el$11, _p$.b = _v$18);
				_v$19 !== _p$.T && setAttribute(_el$11, "aria-label", _p$.T = _v$19);
				_v$20 !== _p$.A && className(_el$12, _p$.A = _v$20);
				_v$21 !== _p$.O && setAttribute(_el$12, "aria-label", _p$.O = _v$21);
				return _p$;
			}, {
				e: void 0,
				t: void 0,
	...
	*/
	...
	var FIT_MODE_ORDER = IMAGE_FIT_MODES.map((mode) => ({
		mode,
		iconName: FIT_MODE_ICONS[mode]
	}));
	...
	*/
	...
	var _tmpl$2$1 = /*#__PURE__*/ template(`<div><div aria-hidden=true></div><div></div><ul data-gallery-element=items>`);
	function VerticalGalleryView(props) {
		...
		return (() => {
			...
			insert(_el$8, createComponent(For, {
				...
				children: (item, index) => createComponent(VerticalImageItem, {
					media: item,
					get index() {
						return index();
					},
					...
					get isFocused() {
						return index() === focus.focusedIndex();
					},
					get forceVisible() {
						return preloadIndices().includes(index());
					},
					fitMode: imageFitMode,
					onClick: () => handleMediaItemClick(index()),
					onMediaLoad: handleMediaLoad,
					get className() {
						return cx(VerticalGalleryView_module_default.galleryItem, index() === currentIndex() && VerticalGalleryView_module_default.itemActive);
					},
					get registerContainer() {
						return createRegisterContainer(index);
					},
					get onFocus() {
						return createHandleFocus(index);
					}
				})
			}));
			createRenderEffect((_p$) => {
	...
	function planBulkDownload(input) {
		return {
			items: input.mediaItems.map((media) => ({
				...
				getBlob: input.mediaBlobProvider ? (signal) => input.mediaBlobProvider?.(media, signal) ?? null : void 0
			})),
	...
	*/
	...
	async function downloadWithFetchFallback(url, filename, options, abortSignal, adapter) {
		...
		const fetchSignalScope = abortSignal ? mergeAbortSignalsWithCleanup([abortSignal, timeoutSignal]) : {
		...
		try {
			...
			const result = abortSignal ? await raceWithAbort(downloadBlobPromise, abortSignal, createAbortResult) : await downloadBlobPromise;
			if (!result.success) return result;
			reportProgress(options.onProgress, {
				phase: "complete",
				current: 1,
			...
			if (abortSignal?.aborted) return createAbortResult();
			const fetchError = timeoutSignal.aborted ? /* @__PURE__ */ new Error(`Download fetch timed out after ${DEFAULT_REQUEST_TIMEOUT_MS}ms`) : error;
	...
	*/
	async function calculateCRC32(data, signal) {
		...
		for (let chunkStart = 0; chunkStart < data.length; chunkStart += CRC32_CHUNK_SIZE) {
			...
			for (let i = chunkStart; i < chunkEnd; i++) crc = crc >>> 8 ^ table[(crc ^ data[i]) & 255];
	...
	var StreamingZipWriter = class {
		...
		*/
		addFile(filename, data, options = {}) {
			const operation = this.pendingAdd.then(() => this.addFileInOrder(filename, data, options.signal));
			this.pendingAdd = operation.catch(() => void 0);
			return operation;
		}
		async addFileInOrder(filename, data, signal) {
			signal?.throwIfAborted();
			...
			while (currentIndex < total) {
				...
				try {
					...
					if (item.blob || item.getBlob) {
						let blob;
						try {
							const provided = item.blob ?? item.getBlob?.(abortSignal) ?? void 0;
		...
		async downloadBulk(mediaItems, options = {}) {
			...
			try {
				...
				const plan = planBulkDownload({
					...
					mediaBlobProvider: options.mediaBlobProvider,
	...
	*/
			...
			try {
				...
				if (type === "current") {
					...
					const result = await downloadService.downloadBulk([...mediaItems], {
						mediaBlobProvider: (item, requestSignal) => mediaService.getDownloadMedia(item, requestSignal),
						signal
					});
					if (!result.success) {
	...
	var _tmpl$$2 = /*#__PURE__*/ template(`<div data-xeg-gallery-container data-pp-product=xeg role=dialog aria-modal=true tabindex=-1>`);
	...
	function GalleryContainer(props) {
		...
		createEffect(() => {
			...
			const handleKeyDown = (event) => {
				...
				const focusableElements = Array.from(containerEl.querySelectorAll(focusableSelectors)).filter((el) => {
					const style = getComputedStyle(el);
					return el.tabIndex >= 0 && el.getClientRects().length > 0 && style.visibility === "visible" && style.display !== "none" && !el.closest("[hidden], [inert], [aria-hidden=\"true\"]");
				});
			...
			if (!scrollRestoration) {
				...
				backgroundObserver = new MutationObserver((records) => {
					for (const record of records) for (const node of record.addedNodes) {
						...
						if (node === containerElement || containerElement !== void 0 && node.contains(containerElement)) continue;
						hideBackground(node);
					}
				});
		...
		return (() => {
			...
			insert(_el$, () => local.children);
			createRenderEffect((_p$) => {
				var _v$ = local.theme ?? "auto", _v$2 = translate("msg.gal.imageGallery"), _v$3 = local.lang ?? "en", _v$4 = local.dir ?? "ltr";
				_v$ !== _p$.e && setAttribute(_el$, "data-pp-theme", _p$.e = _v$);
				_v$2 !== _p$.t && setAttribute(_el$, "aria-label", _p$.t = _v$2);
				_v$3 !== _p$.a && setAttribute(_el$, "lang", _p$.a = _v$3);
				_v$4 !== _p$.o && setAttribute(_el$, "dir", _p$.o = _v$4);
				return _p$;
			}, {
				e: void 0,
				t: void 0,
	...
	*/
	...
	function pruneWithTemplate(input, template, depth = 0) {
		...
		for (const key of Object.keys(template)) {
			...
			if (isRecord$1(tplVal) && !Array.isArray(tplVal)) out[key] = pruneWithTemplate(inVal, tplVal, depth + 1);
			else out[key] = inVal;
		}
		return out;
	}
	...
	*/
	function migrateVideoClickMode(gallery) {
		...
		if (isVideoClickMode(gallery.videoClickMode)) return {};
	...
	function migrateSettings(input, nowMs) {
		...
		if (isRecord$1(input.gallery)) migratedInput = {
			...input,
			gallery: {
				...input.gallery,
		...
		const pruned = pruneWithTemplate(migratedInput, DEFAULT_SETTINGS);
		const merged = {
			...DEFAULT_SETTINGS,
			...pruned
		};
	...
	//#endregion
	//#region src/features/settings/services/settings-validation.ts
	/**
	...
	*/
	...
	function filterForbiddenKeys(obj) {
		for (const key of FORBIDDEN_KEYS) if (Object.hasOwn(obj, key)) delete obj[key];
		return obj;
	}
	/**
	...
	*/
	function normalizeStoredSettings(input, nowMs) {
		if (!isRecord$1(input)) return makeDefaults(nowMs);
		const source = filterForbiddenKeys({ ...input });
		return {
			gallery: normalizeGallery(source.gallery),
			toolbar: normalizeToolbar(source.toolbar),
			download: {},
			accessibility: {},
	...
	function normalizeGallery(raw) {
		...
		if (!isRecord$1(raw)) return { ...d };
		return {
			preloadCount: isFiniteNonNegativeNumber(raw.preloadCount) ? Math.min(raw.preloadCount, 100) : d.preloadCount,
			...
			enableKeyboardNav: isBoolean(raw.enableKeyboardNav) ? raw.enableKeyboardNav : d.enableKeyboardNav,
	...
	function normalizeToolbar(raw) {
		...
		if (!isRecord$1(raw)) return { ...d };
		return { autoHideDelay: isFiniteNonNegativeNumber(raw.autoHideDelay) ? Math.min(raw.autoHideDelay, 6e4) : d.autoHideDelay };
	}
	...
	*/
	function validateSettingValue(key, value) {
		...
		switch (key) {
			...
			case "download":
			case "accessibility":
			case "features": return isRecord$1(value);
			case "gallery.preloadCount": return isFiniteNonNegativeNumber(value) && value <= 100;
			case "gallery.imageFitMode": return isImageFitMode(value);
			case "gallery.theme": return isThemeSetting(value);
			case "gallery.animations":
			case "gallery.enableKeyboardNav":
			case "gallery.videoMuted": return isBoolean(value);
			case "gallery.videoVolume": return isFiniteRange(value, 0, 1);
			case "gallery.videoClickMode": return isVideoClickMode(value);
			case "toolbar.autoHideDelay": return isFiniteNonNegativeNumber(value) && value <= 6e4;
			case "features.gallery":
			case "features.settings":
			case "features.download":
			case "features.mediaExtraction":
			case "features.accessibility": return isBoolean(value);
	...
	function settingsMatch(left, right) {
		...
		if (!isRecord$1(left) || !isRecord$1(right)) return false;
	...
	var PersistentSettingsRepository = class {
		...
		async load() {
			const raw = await this.storage.get(APP_SETTINGS_STORAGE_KEY);
			if (!isRecord$1(raw)) return globalThis.structuredClone(createDefaultSettings(Date.now()));
			const stored = raw;
	...
	var SettingsService = class {
		...
		async set(key, value) {
			if (!validateSettingValue(key, value)) throw new Error(`Invalid setting value for ${key}`);
			const update = this.updateQueue.then(() => this.commitSetting(key, value), () => this.commitSetting(key, value));
			this.updateQueue = update.catch(() => void 0);
			await update;
		}
		async commitSetting(key, value) {
			...
			if (!this.assignNestedPath(nextSettings, key, value)) throw new Error(`Failed to assign setting value for ${key}`);
	...
	*/
	...
	//#endregion
	//#region src/shared/utils/url/history-navigation.ts
	/**
	...
	*/
	function installHistoryNavigationFallback(onNavigate) {
		...
		const replaceState = (data, unused, url) => {
	...
	*/
	...
	else installHistoryNavigationFallback((url) => {
