...
### Epic DOM-EVENT-CLARITY (신규: 2025-10-02)

**목적**: 갤러리 DOM 이벤트 전파 체계 명확화 및 표준화

**배경**:

- 갤러리 DOM 구조 분석 결과, 이벤트 전파 체계가 암묵적이고 문서화되지 않음
- VerticalImageItem 내 다운로드 버튼 클릭 시 `stopPropagation()` 사용
- 향후 추가 인터랙션 요소(공유, 북마크 등) 추가 시 동일 패턴 반복 필요
- 이벤트 버블링 경로가 명확하지 않아 유지보수 어려움

**솔루션 분석**:

| 옵션                       | 설명                               | 장점                          | 단점                         | 위험도    | 선택      |
| -------------------------- | ---------------------------------- | ----------------------------- | ---------------------------- | --------- | --------- |
| A. Event Delegation        | 상위 컨테이너에서 data-role로 구분 | 이벤트 리스너 감소, 성능 향상 | SolidJS 선언적 패턴과 불일치 | 중간      | ❌        |
| B. withEventBoundary HOC   | 명시적 이벤트 차단 래퍼            | 재사용 가능, 테스트 용이      | 추가 추상화 레이어           | 낮음      | ⚠️ 선택적 |
| C. 현재 패턴 유지 + 문서화 | 가이드라인과 테스트로 표준화       | 변경 없음, SolidJS 패턴 유지  | 중복 코드 가능성             | 매우 낮음 | ✅ 채택   |

**선택 근거**:

- 현재 패턴이 SolidJS의 선언적 이벤트 바인딩과 일치
- TDD로 동작 보장 + 문서화로 명확성 확보
- 최소 변경으로 최대 효과 (위험도 최소화)

**구현 계획**:

**Phase 1: 이벤트 전파 테스트 작성 (RED → GREEN)**

- [ ] 테스트 파일: `test/features/gallery/event-propagation.test.tsx`
  - [ ] 다운로드 버튼 클릭이 아이템 선택을 트리거하지 않는지 검증
  - [ ] 오버레이 내 모든 인터랙티브 요소(다운로드 버튼, 컨텍스트 메뉴) 격리 검증
  - [ ] 이벤트 버블링 경로 추적 테스트 (container → itemsContainer → shell)
  - [ ] data-role 속성을 통한 이벤트 타겟 식별 검증
- [ ] 예상 결과: 현재 구현이 이미 올바르게 동작하므로 모든 테스트 GREEN

**Phase 2: 코드 명확화 및 주석 추가 (REFACTOR)**

- [ ] `VerticalImageItem.solid.tsx` 주석 추가:
  ```typescript
  // 이벤트 전파 규칙:
  ...
  ```
- [ ] handleContainerClick 로직에 의도 설명 주석
- [ ] handleDownloadClick의 event.stopPropagation() 이유 명시

**Phase 3: 문서화 (DOCUMENT)**

- [ ] `docs/CODING_GUIDELINES.md`에 "이벤트 처리 규칙" 섹션 추가:
  - 갤러리 컴포넌트 내 이벤트 전파 원칙
  - data-role을 통한 이벤트 타겟 식별 패턴
  - stopPropagation() 사용 가이드라인
  - 테스트 요구사항 (새 인터랙티브 요소 추가 시)
- [ ] VerticalImageItem Props 타입에 JSDoc 추가:
  ```typescript
  /**
   * 아이템 클릭 핸들러
   * @remarks 내부 인터랙티브 요소(다운로드 버튼 등)는 이벤트 전파를 차단하므로,
   * 이 핸들러는 컨테이너 직접 클릭 시에만 호출됩니다.
   */
  onClick?: () => void;
  ```

**Phase 4 (선택적): 이벤트 경계 유틸리티**

- [ ] `src/shared/utils/events/createEventBoundary.ts` 생성 (향후 필요 시):
  ```typescript
  /**
   * 이벤트 전파를 차단하는 핸들러 래퍼
   * @example
   * const handleClick = createEventBoundary((e: MouseEvent) => {
   *   // 이 핸들러는 실행되지만 부모로 전파되지 않음
   *   props.onDownload?.();
   * });
   */
  export function createEventBoundary<E extends Event>(
    handler: (event: E) => void
  ): (event: E) => void;
  ```
- [ ] 테스트 작성 및 문서화
- [ ] 기존 코드에 선택적 적용 (호환성 유지)

**Acceptance Criteria**:

- ✅ 이벤트 전파 테스트 5개 이상 GREEN
- ✅ typecheck 0 errors
- ✅ lint clean
- ✅ 코드 주석으로 이벤트 처리 의도 명확화
- ✅ CODING_GUIDELINES.md에 이벤트 처리 섹션 추가
- ✅ 기존 동작 변경 없음 (회귀 방지)
...
### Epic A11Y-FOCUS-ROLES (신규: 2025-10-02)
