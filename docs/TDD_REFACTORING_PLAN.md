...
**상태**: Phase 1-3 완료, Phase 3-3/3-4 향후 작업
...
- ✅ 다운로드 버튼이 툴바와 아이템 오버레이에 중복 배치되어 혼란 발생 → **해결됨
  (Phase 3-1, 3-2)**
...
- ✅ **Phase 3**: 다운로드 UX 통합
  - 아이템 오버레이 다운로드 버튼 제거 (VerticalImageItem.solid.tsx)
  - 컨텍스트 메뉴 통합 (우클릭 시 onImageContextMenu 호출)
  - 컨텍스트 메뉴 테스트 (11 tests, vertical-image-item.context-menu.test.tsx)
  - 회귀 테스트 수정 (event-propagation, VerticalImageItem.solid-bridge)
  - 브랜치: feature/ux-gallery-feedback-phase3

**남은 작업**:

#### **Phase 3-3: 컨텍스트 메뉴 UI 커스터마이징 (Low Priority, 향후 작업)**

1. **네이티브 컨텍스트 메뉴 대체**
   - 커스텀 컨텍스트 메뉴 컴포넌트 구현 (SolidJS 기반)
   - 다운로드/정보 보기/공유 액션 추가
   - 디자인 토큰 기반 스타일링
   - 접근성 지원 (ARIA attributes, 키보드 네비게이션)

#### **Phase 3-4: 진행률 토스트 토글 툴바 이동 (Low Priority, 향후 작업)**

1. **설정 패널에서 다운로드 워크플로로 이동**
   - 툴바 다운로드 그룹에 서브메뉴 또는 토글 버튼 추가
   - 설정 패널 연동 유지 또는 제거 (논의 필요)
...
- [ ] 커스텀 컨텍스트 메뉴 컴포넌트 구현 완료
- [ ] 다운로드/정보 보기/공유 액션 추가
- [ ] 디자인 토큰 기반 스타일링 적용
- [ ] 접근성 지원 완료 (ARIA, 키보드 네비게이션)
