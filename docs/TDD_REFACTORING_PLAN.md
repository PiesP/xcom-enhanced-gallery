...
**우선순위**: 중간 | **위험도**: 중간 | **상태**: 계획 중

#### 배경

코드베이스 점검 결과 10KB+ 파일 27개 발견:
...
- 코드 모듈화 개선 (단일 책임 원칙)
- 유지보수성 향상 (파일당 300-400줄 목표)
- Tree-shaking 개선 (부수 효과: 빌드 크기 감소 가능)

#### 제안

**Option A: events.ts 분해** (복잡도: 높음)

- `events/listener-manager.ts`: 리스너 추가/제거/관리
- `events/gallery-events.ts`: 갤러리 이벤트 초기화/정리
- `events/event-helpers.ts`: 내부 헬퍼 함수
- 장점: 명확한 책임 분리, tree-shaking 개선
- 단점: 내부 의존성 많아 순환 참조 위험, 작업 시간 2-4시간

**Option B: accessibility-utils.ts 분해** (복잡도: 중간)

- `accessibility/color-contrast.ts`: 색상/명암 계산 (8개 함수)
- `accessibility/keyboard-navigation.ts`: 키보드 접근성 (4개 함수)
- `accessibility/screen-reader.ts`: 스크린 리더 지원 (7개 함수)
- 장점: 독립성 높아 분리 용이, 기능별 명확 구분
- 단점: 작업 시간 1-2시간

**Option C: url-patterns.ts 보류**

- 패턴 정의 집합으로서의 성격이 강함
- 분해 시 import 복잡도 증가, 검색/수정 어려움
- 결정: 현상 유지 (700-800줄은 정의 집합으로 허용 가능)
