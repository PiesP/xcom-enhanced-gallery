...
### Phase 133 — Toast 서비스 API 표준화 (제안)

- 🎯 **목표**: Toast 서비스의 다중 export 별칭을 정리하고 공식 API 1개로
  표준화한다.
- 🔍 **배경**: `unified-toast-manager.ts`에서 다음과 같은 중복 별칭들이 있다:
  - `toastManager`, `ToastService`, `toastService`, `toastController` (모두 같은
    객체)
  - `UnifiedToastManager` (deprecated 클래스)
  - 편의 함수들: `addToast()`, `removeToast()`, `clearAllToasts()` (중복)
- ✅ **완료 기준**:
  1. 공식 export: `toastManager` 1개로 표준화
  2. deprecated 별칭 제거
  3. 마이그레이션 가이드 작성 (주석으로)
  4. 테스트 모두 GREEN
- 📅 **예상 일정**: 선택적 (Phase 131/132 우선순위 높음)

### Phase 134 — Performance/Memory Utilities 사용성 검증 (제안)

- 🎯 **목표**: Performance/Memory 유틸들의 실제 사용처 검증 및 미사용 함수 제거
- 🔍 **조사 대상**:
  - `src/shared/utils/performance/index.ts` (5개 export)
  - `src/shared/utils/memory/index.ts` (정제 완료, 현재 13개 export)
- 📅 **예상 일정**: 선택적
