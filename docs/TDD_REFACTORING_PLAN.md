...
- 기존 테스트 회귀 검증 (2664 tests GREEN)
- 번들 크기 모니터링 (473 KB 상한선)
- Phase별 롤백 포인트 확보

---

### Phase 1: 기본 유틸리티 구축 (Option B + D) — 시작 전

**목표**: Reactive Accessor 및 Singleton Listener Manager 구현
...
**Reactive Accessor 테스트** (`test/unit/utils/reactive-accessor.test.ts`):

```typescript
describe('Reactive Accessor Utilities', () => {
  describe('resolve', () => {
    it('should resolve function accessor', () => {
      const accessor = () => 42;
      expect(resolve(accessor)).toBe(42);
    });
    ...
    it('should preserve type information', () => {
      const accessor: MaybeAccessor<string> = () => 'test';
      const result: string = resolve(accessor);
  ...
  describe('resolveWithDefault', () => {
    it('should return default when undefined', () => {
      expect(resolveWithDefault(undefined, 10)).toBe(10);
    });

    it('should resolve accessor when provided', () => {
      expect(resolveWithDefault(() => 20, 10)).toBe(20);
    });

    it('should return static value when provided', () => {
      expect(resolveWithDefault(30, 10)).toBe(30);
    });
  });

  describe('resolveMultiple', () => {
    it('should resolve array of mixed accessors', () => {
      const result = resolveMultiple(
        () => 1,
        2,
        () => 3
      );
...
2. **Singleton Listener Manager 테스트**
   (`test/unit/utils/singleton-listener.test.ts`):
   ...
   describe('SingletonListenerManager', () => {
     ...
     it('should register and call cleanup', () => {
       const cleanup = vi.fn();
     ...
     it('should handle unregister of non-existent key', () => {
...
1. **Reactive Accessor 구현** (`src/shared/utils/reactive-accessor.ts`):
...
2. **Singleton Listener Manager 구현**
   (`src/shared/utils/singleton-listener.ts`):
   ...
   export class SingletonListenerManager {
     private listeners = new Map<string, CleanupFn>();
...
3. **Barrel Export 추가** (`src/shared/utils/index.ts`):

   ```typescript
   // ... 기존 exports
   export * from './reactive-accessor';
   export * from './singleton-listener';
   ```

#### 1-3. REFACTOR: useGalleryScroll 마이그레이션

파일: `src/features/gallery/hooks/useGalleryScroll.ts`
...
// resolve/resolveWithDefault는 import로 대체
// activeCleanup은 globalListenerManager로 대체

const LISTENER_KEY = 'gallery-wheel';

// 기존 activeCleanup 사용 부분
// Before
if (activeCleanup.fn) {
  activeCleanup.fn();
}
activeCleanup.fn = cleanup;

// After
globalListenerManager.register(LISTENER_KEY, cleanup);
```

**Acceptance (Phase 1)**:

- [ ] 15개 테스트 작성 및 GREEN (Accessor 7개 + Singleton 8개)
- [ ] 구현 완료: `reactive-accessor.ts`, `singleton-listener.ts`
- [ ] useGalleryScroll 마이그레이션 완료
- [ ] Barrel export 추가
- [ ] 타입 체크 GREEN (`npm run typecheck`)
- [ ] 린트 GREEN (`npm run lint:fix`)
- [ ] 기존 테스트 회귀 없음 (2664 tests GREEN)
- [ ] 번들 크기 영향 확인 (±0.5 KB 이내)
