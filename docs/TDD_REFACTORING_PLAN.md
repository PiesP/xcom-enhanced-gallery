# TDD 리팩토링 계획# TDD 리팩토링 계획

**최종 업데이트**: 2025-10-27 | **상태**: 모든 Phase 완료 ✅**마지막 업데이트**:
2025-10-27 | **상태**: Phase 207 완료 ✅
...
| 항목 | 상태 | 비고 || 빌드 | ✅ 안정 | 병렬화 + 메모리 최적화 완료 |

| ------------- | ----------------- | --------------------------- || 성능 | ✅
14.7% 향상 | 병렬 실행으로 7.3초 단축 |

| 빌드 | ✅ 안정 | 병렬화 + 메모리 최적화 완료 || 테스트 | ✅ 82/82 E2E | 모두
통과 (5 skipped) |

| 성능 | ✅ 14.7% 향상 | 병렬 실행으로 7.3초 단축 || 접근성 테스트 | ✅ 33/33
통과 | 3개 파일 통합 완료 |

| 테스트 | ✅ 82/82 E2E | 모두 통과 (5 skipped) || 타입/린트 | ✅ 0 errors |
모두 통과 |

| 접근성 테스트 | ✅ 33/33 통과 | 통합 완료 || 의존성 | ✅ 0 violations | 정책
준수 |

| 타입/린트 | ✅ 0 errors | 모두 통과 || 로컬 빌드 | ✅ 최적화 | 순차 49.5초 →
병렬 42.2초 |

| 의존성 | ✅ 0 violations | 정책 준수 || 번들 크기 | ✅ 341 KB | 목표 ≤345 KB
(4 KB 여유) |

| 번들 크기 | ✅ 341 KB | 목표 ≤345 KB (4 KB 여유) || 상태 | ✅ Phase 207 완료 |
Global Setup 현대화 및 최적화 |
...
모든 Phase가 완료되었습니다. 새로운 리팩토링 작업이 필요한 경우 아래
템플릿을추가하세요.
...
````markdown
### Phase XXX 🔄 (YYYY-MM-DD) - [작업 제목]**목표**: `playwright/smoke` 디렉터리 테스트 파일 통합 및 현대화
...
- 현재 문제점 - 처리 흐름: JSX 변환 → 번들링 → IIFE 생성

- 개선이 필요한 이유 - 의존성: esbuild, @babel/core, babel-preset-solid
...
1. [ ] 작업 항목 1 - `types/babel-preset-solid.d.ts` 생성 (untyped 패키지 선언)

1. [ ] 작업 항목 2 - `@ts-expect-error` 제거 (동적 import로 대체)

1. [ ] 작업 항목 3 - TypeScript strict mode 통과

**수용 기준**:3. **에러 핸들링 추가** ✅

- Babel 변환: try-catch로 JSX 변환 실패 감지

- [ ] 테스트 통과 (unit + integration + E2E) - 빌드 함수: top-level try-catch로
      예외 전파

- [ ] 타입체크/린트 통과 - 상세 에러 메시지: 스택 트레이스 및 컨텍스트 정보

- [ ] 빌드 크기 유지 (≤345 KB)

- [ ] 성능 저하 없음4. **CI/Verbose 로깅** ✅
  - 환경 감지: `process.env.CI` 및 `process.env.VERBOSE`

**예상 시간**: X 시간 - 조건부 출력: CI/verbose 환경에서만 상세 로그
...
## 📚 참고 문서   - 빌드 파이프라인: 정상 작동



- **완료 기록**: [TDD_REFACTORING_PLAN_COMPLETED.md](./TDD_REFACTORING_PLAN_COMPLETED.md)**효과**:

  (Phase 197-207 포함)

- **아키텍처**: [ARCHITECTURE.md](./ARCHITECTURE.md)- 타입 안정성 향상 (@ts-expect-error 제거)

- **코딩 규칙**: [CODING_GUIDELINES.md](./CODING_GUIDELINES.md)- 디버깅 용이성 개선 (상세 에러 로깅)

- **테스트 전략**: [TESTING_STRATEGY.md](./TESTING_STRATEGY.md)- CI 환경 최적화 (환경별 로그 레벨)

- **유지보수**: [MAINTENANCE.md](./MAINTENANCE.md)- 코드 가독성 향상 (명확한 JSDoc)
...
## 💡 추천 다음 단계- `types/babel-preset-solid.d.ts`:

  - babel-preset-solid 모듈 선언 (unknown 타입)

### 성능 최적화  - TypeScript 컴파일러가 untyped 패키지 인식

- `playwright/global-setup.ts`:

- [ ] **번들 크기 추가 최적화** - Tree shaking 개선, 중복 코드 제거  - 37줄 → 125줄 (JSDoc, 에러 핸들링, 로깅 추가)

- [ ] **렌더링 성능** - 불필요한 Effect 제거, memo 최적화  - 동적 import로 babel-preset-solid 로드

- [ ] **네트워크 요청 최적화** - 병렬 요청, 캐싱 전략  - 환경별 로그 레벨 자동 조정
...
- [ ] **테스트 커버리지 향상** - 현재 미테스트 영역 식별 및 추가### Phase 206 ✅ (2025-10-27) - Playwright 스모크 테스트 통합 및 최신화

- [ ] **타입 안전성 강화** - any 타입 제거, strict null checks

- [ ] **접근성 개선** - WCAG AAA 수준 도달**목표**: playwright/smoke 디렉터리의 중복 테스트 통합 및 논리적 그룹화
...
- [ ] **문서 시각화** - 아키텍처 다이어그램, 플로우차트1. **파일 통합** ✅

- [ ] **개발 도구 개선** - 더 빠른 HMR, 더 나은 에러 메시지   - Phase 특화 파일 정리:

- [ ] **CI/CD 최적화** - 캐싱 전략, 병렬 작업 확대     - phase-145-3-loading-timing-e2e.spec.ts 제거 (SKIP 상태, 네트워크 테스트)
...
**작성**: 2025-10-26 | **최종 업데이트**: 2025-10-27       (통합)
