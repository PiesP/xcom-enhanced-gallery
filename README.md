...
- **UI 프레임워크**: SolidJS - 고성능 반응형, 작은 번들 크기
- **번들 크기**: 450KB 미만으로 최적화 (gzip: 111KB)
...
- **아이콘 시스템**: Heroicons → SolidJS 컴포넌트로 안전하게 변환하여
  사용합니다.
...
| 라이브러리  | 라이센스 | 용도                          | 상태               |
| ----------- | -------- | ----------------------------- | ------------------ |
| **SolidJS** | MIT      | UI 프레임워크 (반응형 시스템) | ✅ 현재 사용 중    |
| **fflate**  | MIT      | ZIP 압축 (과거 사용)          | 📦 Historical Only |

> **참고**: `fflate`는 현재 프로젝트에서 제거되었으며, 커스텀 `StoreZipWriter`로
> 대체되었습니다. 라이선스 파일은 과거 기여를 인정하기 위해 보존됩니다.
