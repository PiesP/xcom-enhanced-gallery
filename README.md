...
X.com(트위터)에서 이미지와 동영상을 고품질로 다운로드하고 갤러리 뷰어를 제공하는 유저스크립트입니다.
...
이 저장소의 `dist/xcom-gallery.user.js` 파일을 클릭하여 설치하세요:

🔗 **[dist/xcom-gallery.user.js를 클릭하여 설치](https://github.com/PiesP/xcom-enhanced-gallery/raw/master/dist/xcom-gallery.user.js)**

> 💡 위 링크를 클릭하면 유저스크립트 매니저에서 자동으로 설치 화면이 나타납니다.

#### 방법 2: 릴리스에서 다운로드

[최신 릴리스](https://github.com/PiesP/xcom-enhanced-gallery/releases/latest)에서 `xcom-gallery.user.js` 파일을 다운로드하여 설치하세요.
...
npm run build          # 프로덕션 빌드 (prebuild 검증 포함)
npm run build:dev      # 개발용 빌드 (소스맵 포함)
npm run prebuild       # 빌드 전 종합 검증 (의존성, 타입, 린트, 테스트)
```
...
```bash
# 빠른 검증 (개발 중)
npm run validate:quick     # 타입체크 + 린트 수정 + 포맷팅

# 전체 검증 (커밋 전)
npm run validate:all       # 의존성 + 타입 + 포맷 + 린트 + 단위테스트
npm run validate:strict    # 엄격한 검증 (수정 없이 검사만)
npm run validate:ci        # CI용 검증 (strict + 단위테스트)

# 특별 검증
npm run validate:deps      # 의존성 규칙 검증 + 리포트 생성
npm run quality           # 코드 품질 종합 검증
```
...
npm run test:unit-only    # 단위 테스트만 실행
npm run test:integration  # 통합 테스트 실행
npm run test:e2e          # E2E 테스트 실행
npm run test:watch        # 테스트 감시 모드
npm run test:coverage     # 테스트 커버리지 포함
```
...
```bash
npm run deps:check        # 의존성 규칙 검증
npm run deps:report       # HTML 리포트 생성
npm run deps:graph        # 의존성 그래프 (SVG)
npm run deps:all          # 모든 의존성 분석
```
...
```bash
npm install               # 의존성 설치
npm run validate:quick    # 빠른 검증으로 환경 확인
...
```bash
npm run validate:quick    # 타입체크 + 린트 수정 + 포맷팅
npm run test:unit-only    # 관련 단위 테스트 실행
```

#### 커밋 전 (Pull Request 준비)
...
```bash
npm run build            # 프로덕션 빌드 (자동으로 prebuild 실행)
...
```bash
npm run validate:ci      # 엄격한 검증 + 테스트
npm run deps:all         # 의존성 분석 리포트 생성
```

# 테스트 스크립트

npm run test # 전체 테스트 (통합 + 단위)
npm run test:unit-only # 단위 테스트만 실행
npm run test:watch # 테스트 감시 모드
...
npm run deps:check # 의존성 검증

````

### 빌드 전 검증 워크플로

프로젝트는 빌드 전 자동 품질 검증을 수행합니다:

```bash
# 빌드 시 자동으로 실행되는 prebuild 검증
npm run build
# → npm run deps:check    (의존성 순환 참조 검사)
# → npm run typecheck     (TypeScript 타입 검사)
# → npm run format:check  (코드 포맷팅 검사)
# → npm run lint:fix      (ESLint 규칙 검사 및 자동 수정)
# → npm run test          (전체 테스트 실행)
# → vite build (실제 빌드)
...
# → 의존성, 타입, 포맷 검사, 린트, 단위 테스트
````

프로덕션 빌드 시 자동으로 실행되는 `prebuild` 스크립트는 다음 검사를 순차적으로 수행합니다:
...
1. **의존성 검증** (`deps:check`) - Clean Architecture 의존성 규칙 확인
2. **타입 검사** (`typecheck`) - TypeScript 컴파일 오류 검사
3. **포맷 검사** (`format:check`) - Prettier 포맷팅 규칙 확인
4. **린트 & 수정** (`lint:fix`) - ESLint 규칙 검사 및 자동 수정
5. **테스트 실행** (`test`) - 전체 테스트 스위트 실행

> 💡 **권장사항**: 커밋 전에 `npm run prebuild`를 실행하여 모든 품질 기준을 만족하는지 확인하세요.
