...
| **데이터 새로고침** | 🔄 새로고침      | 추출 실패/불완전 시 미디어 재추출 (로딩 중 회전 표시) |
...
| `1 2 3 4`    | 이미지 맞춤 모드      | 원본 / 가로맞춤 / 세로맞춤 / 창맞춤 |

### 고급 기능

- **이미지 확대/축소**: 마우스 휠로 이미지 세부 사항 확인
- **자동 스크롤**: 긴 이미지의 경우 자동으로 아래쪽까지 스크롤
- **로딩 최적화**: 현재 보는 이미지 우선 로딩으로 빠른 탐색 -- **메모리 관리**:
  사용하지 않는 이미지 자동 해제로 성능 최적화
- **수동 새로고침**: 네트워크/DOM 변동으로 누락된 미디어 재시도 가능 (툴바
  새로고침 버튼)
