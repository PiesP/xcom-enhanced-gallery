...
> Solid.js 기반 U## 테스트 전략 (TDD)

- 환경: Vitest + JSDOM + @solidjs/testing-library, 기본 URL `https://x.com`,
  `test/setup.ts` 자동 로드(폴리필/GM\_\* 모킹/벤더 초기화)
...
- 새 기능은 "실패하는 테스트 → 최소 구현 → 리팩토링" 순서로 진행. 타입은
  명시적으로 선언(strict 유지)프로젝트를 위한 AI 코딩 가이드 (프로젝트 특화, TDD
  > 우선)

## 핵심 스택/워크플로

- Stack: TypeScript(strict) + Vite 7 + Solid.js 1.9 + Vitest 3(JSDOM)
...
- 외부 라이브러리 접근은 오직 getter 경유: `@shared/external/vendors`가 안전
  ...
  - 예)
    `const { createSignal, createEffect } = getSolid(); const { render } = getSolidWeb();`
  - 직접 import 금지: `solid-js`, `solid-js/web` 등을 코드에서 바로 import 하지
    마세요
...
- Features: `gallery/`(주요 UI) · `settings/`(환경설정 UI)
  - 예: `features/gallery/GalleryApp.tsx`, `GalleryRenderer.ts`,
...
- State: `shared/state/**`(Solid Signals), 파생값은
  `@shared/utils/signalSelector.ts`
...
- 상태: Solid Signals 중심(`src/shared/state/**`,
  `@shared/utils/signalSelector.ts` 활용). 컴포넌트에서는 createMemo로 파생값을
  메모이즈
- UI: Solid 컴포넌트. JSX를 사용하고, createSignal/createEffect/createMemo
  primitives 활용
...
- Vendors:
  `import { initializeVendors, getSolid, getSolidWeb } from '@shared/external/vendors'`
- Userscript:
