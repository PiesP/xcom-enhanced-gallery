 ...
 * @param isDev - Whether this is a dev build (determines encoding method)
 *                - true: Plain CSS (escaped with JSON.stringify) - convenient for CSS debugging during development
 *                - false: Base64 encoded - production optimization (better gzip compression rate)
...
function createStyleInjector(css: string, isDev: boolean = false): string {
  ...
  try {
    ...
    // 🔒 Production build: Keep Base64 encoding (optimized)
    // - gzip compression efficiency is about 15-20% better than plain text
    // - Guarantees binary safety
