...
describe('Glass Surface 디자인 일관성 - TDD GREEN Phase (분리된 클래스)', () => {
  ...
  describe('Toolbar 스타일 리팩토링 요구사항', () => {
    it('Toolbar.module.css에서 하드코딩된 glassmorphism 대신 CSS 변수를 사용해야 함', () => {
      const toolbarCSS = readFile('src/shared/components/ui/Toolbar/Toolbar.module.css');

      // CSS 변수를 사용해야 함
      expect(toolbarCSS.includes('backdrop-filter: blur(var(--xeg-glassmorphism-blur))')).toBe(
        true
      );

      // 하드코딩된 값은 제거되어야 함 (CSS 변수 사용 제외)
      const hardcodedBackdropFilter = toolbarCSS.match(/backdrop-filter:\s*blur\([0-9]+px\)/g);
      expect(hardcodedBackdropFilter).toBeNull();
    });
  });

  describe('SettingsModal 스타일 리팩토링 요구사항', () => {
    it('SettingsModal.module.css에서 하드코딩된 glassmorphism 대신 CSS 변수를 사용해야 함', () => {
      const modalCSS = readFile('src/shared/components/ui/SettingsModal/SettingsModal.module.css');

      // CSS 변수를 사용해야 함
      expect(modalCSS.includes('backdrop-filter: blur(var(--xeg-glassmorphism-blur))')).toBe(true);

      // 하드코딩된 값은 제거되어야 함 (CSS 변수 사용 제외)
      const hardcodedBackdropFilter = modalCSS.match(/backdrop-filter:\s*blur\([0-9]+px\)/g);
      expect(hardcodedBackdropFilter).toBeNull();
    });
  });
