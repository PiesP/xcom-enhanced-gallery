 ...
 */
function parseThemeVariables(cssContent, selector) {
  const variables = {};
 ...
 */
function parseThemeVariableNames(cssContent, theme) {
...
describe('테마 일관성 검증', () => {
  ...
  it('툴바와 설정 모달이 동일한 글래스 변수를 사용해야 함', () => {
    ...
    // 둘 다 surface-glass-bg 변수를 사용해야 함
    expect(toolbarCss).toMatch(/var\(--xeg-surface-glass-bg\)/);
    expect(modalCss).toMatch(/var\(--xeg-surface-glass-bg\)/);

    // 둘 다 surface-glass-border 변수를 사용해야 함
    expect(toolbarCss).toMatch(/var\(--xeg-surface-glass-border\)/);
    expect(modalCss).toMatch(/var\(--xeg-surface-glass-border\)/);
  });
});
