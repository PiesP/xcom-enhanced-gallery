...
describe('Phase D: unified-utils.ts 간소화', () => {
  describe('1. 파일 분리 및 간소화', () => {
    ...
    it('1116라인의 거대한 파일이 분리되어야 한다', async () => {
      ...
      try {
        ...
        // Gallery utilities
        const galleryUtils = await import('@shared/utils/gallery');
        expect(galleryUtils).toBeDefined();
      } catch (error) {
        // 아직 분리되지 않았을 수 있음
        expect(error).toBeDefined();
      }
  ...
  describe('2. 네이밍 간소화', () => {
    it('"unified" 접두사가 모든 곳에서 제거되어야 한다', async () => {
      ...
      const unifiedExports = exportNames.filter(name => 
        name.toLowerCase().includes('unified')
      );
    ...
    it('복잡한 수식어들이 제거되어야 한다', async () => {
      ...
      forbiddenWords.forEach(word => {
        const hasWord = exportNames.some(name => 
  ...
  describe('3. 복잡도 감소', () => {
    it('Debouncer 클래스가 간단한 팩토리 함수로 충분해야 한다', async () => {
      ...
      const debouncedFn = createDebouncer(testFn, 10);
