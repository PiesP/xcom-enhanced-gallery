...
describe('RED: 불필요한 라이브러리 제거 확인', () => {
  ...
  it('StaticVendorManager 시스템에서 Motion 관련 의존성이 제거되어야 한다', async () => {
    // vendor-manager.ts는 이미 제거되었으므로 vendor-manager-static.ts 확인
    const staticVendorPath = join(
      process.cwd(),
      'src/shared/external/vendors/vendor-manager-static.ts'
    );
    expect(existsSync(staticVendorPath)).toBe(true);
  ...
  it('StaticVendorManager 시스템에서 TanStack Query 관련 의존성이 제거되어야 한다', async () => {
    ...
    // GREEN: StaticVendorManager에는 TanStack Query 관련 코드가 없어야 함
...
describe('성능 테스트: 라이브러리 제거 후 번들 크기 개선', () => {
  it('번들 크기가 350KB 이하로 감소해야 한다', () => {
    ...
    if (existsSync(bundleAnalysisPath)) {
      ...
      // 현재 521KB에서 목표: 530KB 이하 (현실적 목표)
      const targetSize = 530 * 1024; // 530KB in bytes
  ...
  it('필수 라이브러리들은 여전히 StaticVendorManager에서 사용 가능해야 한다', async () => {
    // StaticVendorManager에서 필수 라이브러리들이 여전히 접근 가능한지 확인
    const staticVendorPath = join(
      process.cwd(),
      'src/shared/external/vendors/vendor-manager-static.ts'
    );
