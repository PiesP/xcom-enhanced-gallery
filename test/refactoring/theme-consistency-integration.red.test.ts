/**
 * Theme Consistency Integration Test - TDD GREEN/REFACTOR Phase
 *
 * ✅ 테마 일관성 통합 테스트
...
describe('✅ Theme Consistency Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    if (!document.querySelector('#test-design-tokens')) {
      ...
      styleElement.innerHTML = `
        ...
        /* 글래스모피즘 스타일 */
        [data-theme-style='glassmorphism'] .toolbar,
        [data-theme-style='glassmorphism'] .settings-modal,
        [data-theme-style='glassmorphism'] .gallery-container,
        [data-theme-style='glassmorphism'] .tooltip,
        [data-theme-style='glassmorphism'] .coach-mark,
        [data-theme-style='glassmorphism'] [data-component-type] {
          backdrop-filter: blur(20px) !important;
          background: var(--xeg-glass-background) !important;
          border: 1px solid rgba(255, 255, 255, 0.2) !important;
          border-radius: var(--xeg-border-radius) !important;
        }

        /* 네이티브 테마에서 글래스모피즘 비활성화 */
        [data-theme-style='native'] .toolbar,
        [data-theme-style='native'] .settings-modal,
        [data-theme-style='native'] .gallery-container,
        [data-theme-style='native'] .tooltip,
        [data-theme-style='native'] .coach-mark,
        [data-theme-style='native'] [data-component-type],
        [data-theme="native"] .toolbar,
        [data-theme="native"] .settings-modal,
        [data-theme="native"] .gallery-container,
        [data-theme="native"] .tooltip,
        [data-theme="native"] .coach-mark,
        [data-theme="native"] [data-component-type] {
          backdrop-filter: none !important;
          background: var(--xeg-native-background) !important;
          border: 1px solid var(--xeg-native-border) !important;
        }
  ...
  describe('Global Theme Application', () => {
    it('should apply theme consistently across all component types', () => {
      ...
      themeService.setTheme('dark'); // native 대신 dark 사용
      ...
      componentTypes.forEach(componentType => {
        ...
        // 단순화된 테마 시스템: 네이티브 스타일이 기본
        expect(mockComponent.getAttribute('data-theme')).toBe('dark');
        // data-theme-style 속성은 더 이상 사용하지 않음 (네이티브 스타일이 기본)
        expect(mockComponent.getAttribute('data-theme-style')).toBeNull();
      });
    ...
    it('should handle theme switching with immediate visual feedback', () => {
      ...
      themes.forEach(theme => {
        ...
        // 갤러리 컨테이너에도 반영되어야 함
        expect(testContainer.getAttribute('data-theme')).toBe(expectedTheme);
      });

      // RED: GREEN 단계에서 해결될 예정
      // expect(false).toBe(true);
    });

    it('should maintain consistent CSS variable values across themes', () => {
      ...
      themes.forEach(theme => {
        ...
        // data-theme-style 속성은 더 이상 사용하지 않음 (네이티브 스타일 기본)
  ...
  describe('Component-Specific Theme Consistency', () => {
    it('should apply glassmorphism consistently across all components', () => {
      ...
      componentsToTest.forEach(({ type }) => {
        ...
        // data-theme-style 속성은 더 이상 사용하지 않음 (네이티브 스타일 기본)
        const themeStyle = element.getAttribute('data-theme-style');
        expect(themeStyle).toBeNull();
      });
    });
    ...
    it('should apply X.com native styling consistently', () => {
      ...
      componentTypes.forEach(componentType => {
        ...
        // data-theme-style 속성은 더 이상 사용하지 않음 (네이티브 스타일 기본)
        const themeStyle = component.getAttribute('data-theme-style');
        expect(themeStyle).toBeNull();
  ...
  describe('Settings Menu Theme Integration', () => {
    it('should show all available theme options in settings', () => {
      ...
      const styledSelector = settingsModal.querySelector('[data-testid="theme-style-selector"]');
      expect(styledSelector).toBeNull();
    ...
    it('should provide theme preview in settings', () => {
      ...
      themes.forEach(theme => {
        ...
        const appliedTheme = mockPreviewElement.getAttribute('data-theme');
        expect(['auto', 'light', 'dark']).toContain(appliedTheme || theme);

        // 미리보기 텍스트 업데이트 (실제 구현은 GREEN에서)
        // const expectedText =
        //   theme === 'auto' ? '시스템 테마' : theme === 'light' ? '라이트 모드' : '다크 모드';
  ...
  describe('Component Lifecycle Theme Consistency', () => {
    it('should apply theme to dynamically added components', () => {
      ...
      // data-theme-style 속성은 더 이상 사용하지 않음 (네이티브 스타일 기본)
      const themeStyle = dynamicComponent.getAttribute('data-theme-style');
      expect(themeStyle).toBeNull();
      ...
      // data-theme-style 속성은 더 이상 사용하지 않음 (네이티브 스타일 기본)
      const anotherThemeStyle = anotherDynamicComponent.getAttribute('data-theme-style');
      expect(anotherThemeStyle).toBeNull();
    });
  ...
  describe('Performance and Error Handling', () => {
    ...
    it('should gracefully handle localStorage errors', () => {
      ...
      // data-theme-style 속성은 더 이상 사용하지 않음 (네이티브 스타일 기본)
      const themeStyle = testElement.getAttribute('data-theme-style');
      expect(themeStyle).toBeNull();
