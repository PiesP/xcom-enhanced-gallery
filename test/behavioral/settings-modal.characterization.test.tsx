...
describe('SettingsModal - 현행 기능 특성화 (Characterization)', () => {
  const defaultProps = {
    isOpen: true,
    onClose: vi.fn(),
    position: 'toolbar-below' as const,
  };
  ...
  const renderModal = (props = defaultProps) => render(() => <SettingsModal {...props} />);

  describe('기본 렌더링', () => {
    it('isOpen=false일 때 렌더링되지 않는다', () => {
      const closedProps = { ...defaultProps, isOpen: false };
      renderModal(closedProps);
  ...
  describe('위치 설정', () => {
    it('position prop에 따라 올바른 data-position 속성이 설정된다', () => {
      const props = { ...defaultProps, position: 'top-right' as const };
      renderModal(props);
    ...
    it('기본 position은 center로 설정된다', () => {
      ...
      renderModal(propsWithoutPosition);
