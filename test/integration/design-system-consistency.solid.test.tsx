...
describe('디자인 시스템 일관성 테스트', () => {
  ...
  describe('색상 일관성', () => {
    it('모든 primary 버튼이 동일한 색상을 사용해야 함', () => {
      ...
      const primaryButtonKeys = ['nav-previous', 'nav-next', 'download-current', 'download-all'];
      const primaryButtons = primaryButtonKeys
        .map(key => container.querySelector<HTMLElement>(`[data-gallery-element="${key}"]`))
        .filter((button): button is HTMLElement => Boolean(button));

      expect(primaryButtons.length).toBeGreaterThan(0);

      if (primaryButtons.length === 1) {
        const [singleButton] = primaryButtons;
        expect(getComputedStyle(singleButton).backgroundColor).toBeTruthy();
        return;
      }
