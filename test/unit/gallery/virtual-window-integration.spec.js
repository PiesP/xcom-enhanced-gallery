/**
 * Phase 2 GREEN: useVirtualWindow 통합 기본 동작 검증 (JS fallback)
 */
  ...
  it.skip('플래그 ON이면 300 아이템 중 화면 윈도우만 렌더 (Phase 11 집중 위해 임시 skip)', async () => {
    ...
    try {
      if (typeof globalThis !== 'undefined' && globalThis.document)
        list = globalThis.document.querySelector('[data-xeg-role="items-list"]');
    } catch {
      /* ignore */
    }
    expect(list).not.toBeNull();
    const childCount = list ? list.children.length : 0;
    expect(childCount).toBeLessThanOrEqual(30); // viewport + overscan 예상 상한
  });
});
