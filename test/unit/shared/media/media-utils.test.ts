...
describe("media-utils", () => {
  ...
  describe("extractVisualIndexFromUrl", () => {
    it("should extract visual index from photo URL", () => {
      expect(
        extractVisualIndexFromUrl("https://x.com/user/status/123/photo/1"),
      ).toBe(0);
      expect(
        extractVisualIndexFromUrl("https://x.com/user/status/123/photo/2"),
      ).toBe(1);
      expect(
        extractVisualIndexFromUrl("https://x.com/user/status/123/photo/4"),
      ).toBe(3);
    });

    it("should extract visual index from video URL", () => {
      expect(
        extractVisualIndexFromUrl("https://x.com/user/status/123/video/1"),
      ).toBe(0);
    });

    it("should return 0 for invalid URLs", () => {
      expect(extractVisualIndexFromUrl("")).toBe(0);
      expect(extractVisualIndexFromUrl("https://x.com/user/status/123")).toBe(
        0,
      );
    ...
    it("should return 0 for non-numeric index", () => {
      expect(
        extractVisualIndexFromUrl("https://x.com/user/status/123/photo/abc"),
      ).toBe(0);
    });
  });
  ...
  describe("extractDimensionsFromUrl", () => {
    it("should extract dimensions from URL pattern", () => {
      expect(
        extractDimensionsFromUrl(
          "https://pbs.twimg.com/media/img.jpg?name=1200x800",
        ),
      ).toBeNull(); // Query param not handled by regex
      expect(
        extractDimensionsFromUrl(
          "https://pbs.twimg.com/media/1200x800/img.jpg",
        ),
    ...
    it("should return null for invalid patterns", () => {
      expect(
        extractDimensionsFromUrl("https://example.com/image.jpg"),
      ).toBeNull();
  ...
  describe("normalizeMediaUrl", () => {
    it("should extract filename from URL", () => {
      expect(normalizeMediaUrl("https://example.com/path/image.jpg")).toBe(
        "image",
      );
      expect(
        normalizeMediaUrl("https://example.com/path/image.jpg?foo=bar"),
      ).toBe("image");
      expect(normalizeMediaUrl("https://example.com/path/image.jpg#hash")).toBe(
        "image",
      );
    });
  ...
  describe("adjustClickedIndexAfterDeduplication", () => {
    ...
    it("should clamp out of bounds index", () => {
      ...
      expect(adjustClickedIndexAfterDeduplication(original, unique, 100)).toBe(
        0,
      );
      expect(adjustClickedIndexAfterDeduplication(original, unique, -1)).toBe(
        0,
      );
    });
