...
describe('Translator class and helpers', () => {
  it('creates instance and lists all supported languages', () => {
    const catalog = new TranslationCatalog();
    const translator = new Translator(catalog);
    const langs = translator.languages;
    expect(Array.isArray(langs)).toBe(true);
    // All supported languages should be listed (including lazy-loadable ones)
    expect(langs).toContain('en');
    expect(langs).toContain('ko');
    expect(langs).toContain('ja');
  });

  it('translates known and unknown keys and interpolates params', async () => {
    const translator = new Translator();

    // English is bundled, should work immediately
    expect(translator.translate('en', 'toolbar.next')).toBe('Next');
    expect(translator.translate('en', 'toolbar.previous')).toBe('Previous');

    // Lazy load ko and ja before testing translations
    await translator.ensureLanguage('ko');
    await translator.ensureLanguage('ja');
...
describe("Translator", () => {
  ...
  it("should initialize with catalog instance", () => {
    const translator = new Translator(catalog);
    // languages now returns all LANGUAGE_CODES (including lazy-loadable)
    expect(translator.languages).toContain("en");
    expect(translator.languages).toContain("ko");
    expect(translator.languages).toContain("ja");
  });
