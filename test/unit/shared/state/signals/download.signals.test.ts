...
describe('Download Signals', () => {
  ...
  describe('createDownloadTask', () => {
    it('should create a download task with provided media info', () => {
      ...
      expect(getResultData(result)).toMatch(/^dl_/);
    });
  ...
  describe('Download Lock', () => {
    ...
    it('should handle nested locks', () => {
      const release1 = acquireDownloadLock();
      const release2 = acquireDownloadLock();

      expect(isDownloadLocked()).toBe(true);

      release1();
      expect(isDownloadLocked()).toBe(true); // Still locked by release2
