...
describe('Phase C: 코드 품질 개선', () => {
  ...
  describe('사용하지 않는 기능 제거 검증', () => {
    it('AdvancedMemoization이 실제로 사용되는지 확인', async () => {
      ...
      const advancedMemo = await import('../../../src/shared/components/optimization/AdvancedMemoization');
    ...
    it('BundleOptimizer가 실제로 사용되는지 확인', async () => {
      const bundleOptimizer = await import('../../../src/shared/utils/optimization/BundleOptimizer');
  ...
  describe('성능 최적화 검증', () => {
    it('중복 제거된 유틸리티들이 올바르게 동작해야 한다', async () => {
      const { removeDuplicates, removeDuplicateStrings } = await import('../../../src/shared/utils/unified-utils');
