...
vi.mock('@/shared/utils/media/media-url.util', () => ({
  ...
  canExtractOriginalImage: vi.fn((url: string) => {
    return url.includes('pbs.twimg.com') && url.includes('/media/');
  }),
  isEmojiUrl: vi.fn((url: string) => {
    return url.includes('/emoji/');
  }),
  isVideoThumbnailUrl: vi.fn((url: string) => {
    return url.includes('/amplify_video_thumb/') || url.includes('/tweet_video_thumb/');
  }),
  extractOriginalVideoUrl: vi.fn((url: string) => url),
  canExtractOriginalVideo: vi.fn((url: string) => {
