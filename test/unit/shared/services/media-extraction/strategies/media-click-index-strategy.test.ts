...
describe('FallbackStrategy', () => {
  ...
  it('should return 0 when mediaItems exist', () => {
    const mediaItems = [createMockMediaInfo(), createMockMediaInfo()];
    const result = strategy.calculate(document.createElement('div'), [], mediaItems);
    expect(result).toBe(0);
  });

  it('should return -1 when mediaItems is empty', () => {
    const result = strategy.calculate(document.createElement('div'), [], []);
    expect(result).toBe(-1);
  });

  it('should have correct confidence level', () => {
    expect(strategy.confidence).toBe(50);
  });
