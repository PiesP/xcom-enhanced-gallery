...
describe('DOMDirectExtractor - 규칙 유틸 통합', () => {
  ...
  it('이미지 URL을 중앙 유틸 규칙대로 orig 품질로 변환해야 한다', async () => {
    const { DOMDirectExtractor } = await import(
      '@shared/services/media-extraction/extractors/dom-direct-extractor.js'
    );
    const { extractOriginalImageUrl } = await import('@shared/utils/media/media-url.util.js');

    const container = document.createElement('article');
    container.setAttribute('data-testid', 'tweet'); // 트윗 컨테이너 마킹
    const img = document.createElement('img');
    img.src = 'https://pbs.twimg.com/media/AbCdEf.jpg?format=jpg&name=large';
    container.appendChild(img);
    document.body.appendChild(container); // DOM에 연결
    ...
    document.body.removeChild(container); // 정리
  });
  ...
  it('우선순위 선택자 기반으로 가장 가까운 상위 트윗 컨테이너(article[data-testid="tweet"])를 선택해야 한다', async () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      // 현재 구현은 클릭된 컨텍스트 내 이미지를 우선시하므로 클릭된 이미지가 추출되어야 함
      expect(result.mediaItems.length).toBe(1);
      expect(result.mediaItems[0].url).toContain('Inner');
    }
  });
});
