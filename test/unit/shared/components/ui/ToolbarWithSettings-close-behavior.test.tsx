 ...
 *
 * TODO: 설정 모달 렌더링 문제 해결 필요
 * - 아이콘 lazy loading과 SolidJS 반응형 시스템의 상호작용 이슈
 * - @solidjs/testing-library의 render와 상태 업데이트 타이밍 문제
 * - 임시로 skip 처리하고 별도 이슈로 추적 예정
 */
...
describe.skip('ToolbarWithSettings - 설정 모달이 열린 상태에서 닫기 동작 [SKIPPED: Icon loading issue]', () => {
      ...
      return createRoot(async disposeFn => {
        ...
        await waitFor(
          () => {
            ...
            // 디버깅: 렌더링된 DOM 구조 확인
            if (!settingsModal) {
              // data-position 속성을 가진 dialog 요소 찾기 (SettingsModal panel 모드)
              const dialogs = document.querySelectorAll('[role="dialog"]');
              if (dialogs.length > 0) {
                // Dialog는 있지만 testId가 없는 경우
                expect(dialogs.length).toBeGreaterThan(0);
                const dialog = dialogs[0] as HTMLElement;
                // 직접 testId 확인
                expect(dialog.getAttribute('data-testid')).toBe('toolbar-settings-modal');
              }
            }
