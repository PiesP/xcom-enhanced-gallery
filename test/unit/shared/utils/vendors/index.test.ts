...
describe('Vendor Library Utilities', () => {
  ...
  describe('Error Handling', () => {
    it('should handle missing vendor gracefully', async () => {
      // window와 global 객체에서 fflate를 모두 제거
      const originalWindowFflate = (window as any).fflate;
      const originalGlobalFflate = (global as any).fflate;
      const originalGlobalThisFflate = (globalThis as any).fflate;

      delete (window as any).fflate;
      delete (global as any).fflate;
      delete (globalThis as any).fflate;

      try {
        // 모듈을 다시 import하고 에러가 발생하는지 테스트
        vi.doUnmock('../../../../../src/shared/utils/vendors/index');
        const { getFflate } = await import('../../../../../src/shared/utils/vendors/index');

        expect(() => getFflate()).toThrow();
      } finally {
        // 원래 상태로 복원
        if (originalWindowFflate) {
          (window as any).fflate = originalWindowFflate;
        }
        if (originalGlobalFflate) {
          (global as any).fflate = originalGlobalFflate;
        }
        if (originalGlobalThisFflate) {
          (globalThis as any).fflate = originalGlobalThisFflate;
        }
      }
    });
  });
