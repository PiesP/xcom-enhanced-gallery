...
describe('Radius Policy', () => {
  ...
  test('Toolbar/Button: px 기반 radius 하드코딩 금지', () => {
    ...
    const toolbarMatches = filterDefined(
      [...toolbarCss.matchAll(hardcoded)].map(match => match[1])
    );
    const buttonMatches = filterDefined([...buttonCss.matchAll(hardcoded)].map(match => match[1]));
  ...
  test('Toolbar: 허용된 토큰만 사용', () => {
    ...
    const tokenUsage = filterDefined(
      Array.from(toolbarCss.matchAll(/border-radius:\s*var\((--[a-z0-9-]+)\)/g)).map(
        match => match[1]
      )
    );
    const disallowed = tokenUsage.filter(token => !allowed.includes(token));
    expect(disallowed).toEqual([]);
  });

  test('Button: primary 변형 radius 정책 준수 (md/lg 조합 허용 범위)', () => {
    const buttonTokenUsage = filterDefined(
      Array.from(buttonCss.matchAll(/border-radius:\s*var\((--[a-z0-9-]+)\)/g)).map(
        match => match[1]
      )
    );
    ...
    const invalid = buttonTokenUsage.filter(token => !allowed.includes(token));
