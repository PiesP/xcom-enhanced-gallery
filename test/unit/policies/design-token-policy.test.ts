...
describe('Design Token Policy - Unified', () => {
  ...
  describe('3. Unused legacy tokens detection', () => {
    it('should not have unused overlay aliases without color- prefix', () => {
      // Phase 351+: Design tokens split into 3-layer structure
      const TOKEN_FILES = [
        toPosix(join(ROOT, 'src/shared/styles/design-tokens.primitive.css')),
        toPosix(join(ROOT, 'src/shared/styles/design-tokens.semantic.css')),
        toPosix(join(ROOT, 'src/shared/styles/design-tokens.component.css')),
      ];
      const TARGET_TOKENS = [
        '--xeg-overlay-light',
        '--xeg-overlay-medium',
      ...
      // Collect all token declarations from 3 files
      const tokensCss = TOKEN_FILES.map(f => readFileSync(f, 'utf8')).join('\n');
      ...
      for (const token of TARGET_TOKENS) {
        ...
        for (const f of files) {
          ...
          if (TOKEN_FILES.includes(toPosix(f))) continue;
