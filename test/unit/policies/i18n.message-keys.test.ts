...
describe('i18n.message-keys (Phase 4 완료 GREEN)', () => {
  ...
  it('한국어 사용자 노출 literal이 소스 코드(언어 리소스 제외)에 남아 있지 않다', () => {
    ...
    for (const file of files) {
      ...
      // Phase 323-1: Exclude logger statements (logger.debug, logger.info, logger.warn, logger.error)
      // Logger messages can contain Korean for debugging purposes
      content = content.replace(/logger\.(debug|info|warn|error)\([^)]*\)/g, '');
