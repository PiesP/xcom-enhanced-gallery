...
describe('Phase C: API 추출 재시도/타임아웃 정책', () => {
  ...
  it.skip('maxRetries만큼 재시도 후 실패하면 DOM 폴백으로 전환한다', async () => {
    ...
    article.setAttribute('data-testid', 'tweet'); // 트윗 컨테이너 마킹
    ...
    document.body.appendChild(article); // DOM에 연결
    ...
    expect(result.status).toBe('success'); // DOM 폴백 성공 기대
    if (result.status === 'success') {
      expect(result.mediaItems.length).toBeGreaterThan(0);
    }

    document.body.removeChild(article); // 정리
    spy.mockRestore();
  });

  it.skip('요청이 타임아웃되면 재시도하고, 최종 실패 시 DOM 폴백한다', async () => {
    const svc = new MediaExtractionService();
    ...
    article.appendChild(link);
    article.appendChild(img);
    (globalThis as any).document.body.appendChild(article); // DOM에 연결
    ...
    (globalThis as any).document.body.removeChild(article); // 정리
    spy.mockRestore();
  });
});
