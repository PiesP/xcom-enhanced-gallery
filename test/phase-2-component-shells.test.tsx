...
import { ToolbarShell } from '../src/shared/components/ui/ToolbarShell';
import { ModalShell } from '../src/shared/components/ui/ModalShell';
...
describe('Phase 2: ModalShell 컴포넌트', () => {
  ...
  test('isOpen=true일 때 올바른 구조로 렌더링되어야 함', () => {
    const { container } = render(() => (
      <ModalShell isOpen data-testid='test-modal'>
        Modal Content
      </ModalShell>
    ));
  ...
  test('surfaceVariant prop에 따른 클래스가 적용되어야 함', () => {
    const { container } = render(() => (
      <ModalShell isOpen surfaceVariant='elevated'>
        Content
      </ModalShell>
    ));
  ...
  test('aria-label이 올바르게 설정되어야 함', () => {
    const { container } = render(() => (
      <ModalShell isOpen aria-label='Custom Modal'>
        Content
      </ModalShell>
    ));
