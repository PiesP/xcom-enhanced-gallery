 ...
 */
export function extractLightness(oklchString: string): number {
 ...
 */
export function lightnessToRelativeLuminance(lightness: number): number {
 ...
 * 컴포넌트 색상 조합의 대비 체크
 */
export interface ComponentContrastCheck {
  component: string;
  combinations: Array<{
    name: string;
    foreground: string;
 ...
 * 다중 색상 조합 대비 체크
 */
export function checkComponentContrast(
  component: string,
