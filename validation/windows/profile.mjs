...
export async function run({ browser, root, output }) {
  ...
  try {
    ...
    const html = await readFile(path.join(root, 'test/e2e/fixtures/mock-gallery-page.html'), 'utf8');
    ...
    const images = await page.evaluate(() => [[480,720],[1200,360],[640,480]].map(([width,height], index) => {
      const canvas = document.createElement('canvas');
      canvas.width=width; canvas.height=height;
      const ctx=canvas.getContext('2d');
      ctx.fillStyle=['#154c72','#73501b','#254b39'][index]; ctx.fillRect(0,0,width,height);
      ctx.strokeStyle='#e5ecf2'; ctx.lineWidth=8; ctx.strokeRect(12,12,width-24,height-24);
      ctx.fillStyle='#ffffff'; ctx.font='28px Segoe UI'; ctx.fillText(`Windows fixture ${index+1}`,30,60);
    ...
    await context.route('**/*', async route => {
      ...
      if (url.hostname==='pbs.twimg.com') {
        ...
        return route.fulfill({contentType:'image/png',body:pngs[index],headers:{'Access-Control-Allow-Origin':'*'}});
      }
      return route.abort('blockedbyclient');
    });
    await page.goto('https://x.com/testuser/status/1234567890123456789');
    const {installGMMock}=await import(pathToFileURL(path.join(root,'test/e2e/fixtures/gm-mock.ts')).href);
    ...
    await page.evaluate(encodedFixtures => {
      ...
      window.GM_xmlhttpRequest = details => {
        queueMicrotask(() => details.onerror?.({ status: 503, statusText: 'Fixture API disabled' }));
      ...
      window.GM_download=(details,name)=>{
        ...
        try {
          ...
          anchor.href=url;anchor.download=options.filename??options.name??'fixture.png';
    ...
    await page.waitForFunction(()=>[...document.querySelectorAll('[data-xeg-gallery-container] img')].some(image=>image.complete&&image.naturalWidth>1));
    ...
    await page.waitForFunction(previous=>document.querySelector('[data-xeg-gallery-container] [role="progressbar"]')?.getAttribute('aria-valuenow')!==previous, initial);
    const next=await progress.getAttribute('aria-valuenow');
    assert.notEqual(next,initial,'ArrowRight must change the selected image');
    await page.screenshot({path:path.join(output,'gallery-panorama.png')});
    const downloadPromise = page.waitForEvent('download', { timeout: 15_000 });
    await page.locator('[data-gallery-element="toolbar"] button[aria-label="Download"]').click();
    await page.waitForFunction(() => Boolean(document.documentElement.dataset.xegAcceptanceDownload));
    ...
    assert(pngs.some(png=>png.equals(bytes)),'Downloaded fixture bytes must match a served PNG');
    await page.keyboard.press('Escape');
    await gallery.waitFor({state:'detached'});
    assert.deepEqual(errors,[]);
    return {checks:['production-userscript-injection','gallery-open','keyboard-next','mock-GM-browser-download','escape-close'],browserVersion:browser.version(),download:{file:filename,suggestedFilename:download.suggestedFilename(),bytes:bytes.length,sha256:createHash('sha256').update(bytes).digest('hex')},scope:'fixture userscript rendering and mocked-GM browser download; no extension installation or live X.com',visualReview:'pending'};
