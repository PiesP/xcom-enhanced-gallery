...
const importReplacements = [
  ...
  {
    oldImport: /import\s*\{\s*ToolbarButton\s*(?:,\s*[^}]+)?\s*\}\s*from\s*['"]([^'"]*\/Toolbar\/ToolbarButton)['"];?/g,
    newImport: `import { Button } from '@shared/components/ui/Button';`
  },
  {
    oldImport: /import\s*\{\s*ToolbarButton\s*as\s+(\w+)\s*\}\s*from\s*['"]([^'"]*\/Toolbar\/ToolbarButton)['"];?/g,
    newImport: `import { Button as $1 } from '@shared/components/ui/Button';`
  },
  
  // IconButton imports  
  {
    oldImport: /import\s*\{\s*IconButton\s*(?:,\s*[^}]+)?\s*\}\s*from\s*['"]([^'"]*\/primitive\/IconButton)['"];?/g,
    newImport: `import { Button } from '@shared/components/ui/Button';`
  },
  {
    oldImport: /import\s*\{\s*IconButton\s*as\s+(\w+)\s*\}\s*from\s*['"]([^'"]*\/primitive\/IconButton)['"];?/g,
    newImport: `import { Button as $1 } from '@shared/components/ui/Button';`
  },
  
  // Button-legacy imports
  {
    oldImport: /import\s*\{\s*Button\s*(?:,\s*[^}]+)?\s*\}\s*from\s*['"]([^'"]*\/Button-legacy\/Button)['"];?/g,
    newImport: `import { Button } from '@shared/components/ui/Button';`
  }
];
