...
async function measureTotalCSS() {
  const allCSSPatterns = ['src/**/*.css', 'src/**/*.module.css'];
...
function validatePerformanceGoals(currentTotal, buttonSize) {
  ...
  console.log('\n🎯 성능 분석:');
  console.log(
    `Button CSS 비율: ${buttonPercentage}% (${buttonSize.totalSizeKB}KB / ${currentTotal.totalSizeKB}KB)`
  );

  // 15% 감소 목표 시뮬레이션 (이전 상태 추정)
  const estimatedPreviousSize = buttonSize.totalSize / 0.85; // 15% 감소했다고 가정
  const actualReduction = (
    ((estimatedPreviousSize - buttonSize.totalSize) / estimatedPreviousSize) *
    100
  ).toFixed(2);
...
async function runMetrics() {
  ...
  try {
    ...
    fs.writeFileSync('metrics/button-consolidation-metrics.json', JSON.stringify(metrics, null, 2));
